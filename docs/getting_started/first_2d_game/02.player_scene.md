# Creating the player scene {#doc_your_first_2d_game_player_scene}
...
functionality -what the object *is*. In the upper-left corner, in the
\"Scene\" tab, click the \"Other Node\" button and add an
...
**warning icon** next to it in the scene tree:

![image](img/no_shape_warning.webp)

This warning tells us that the `Area2D` node requires a shape to detect
collisions or overlaps. We can **ignore the warning temporarily**
because we will first set up the player\'s visuals (using an animated
sprite). Once the visuals are ready, we will add a collision shape as a
child node. This will allow us to accurately size and position the shape
based on the sprite\'s appearance.
...
Save the scene as `player.tscn`. Click **Scene \> Save**, or press
...
list of the animations it can display. Make sure `AnimatedSprite2D` is
selected and then find the `Sprite Frames` property under the
`Animation` section in the Inspector and click \"\[empty\]\" -\> \"New
SpriteFrames\":
...
folder you unzipped earlier. Drag the two images for each animation,
...
Once this is done, the warning on the `Area2D` node will disappear, as
it now has a shape assigned and can interact with other objects.
