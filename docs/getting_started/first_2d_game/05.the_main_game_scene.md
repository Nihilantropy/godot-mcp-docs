# The main game scene {#doc_your_first_2d_game_the_main_game_scene}
...
Scene dock, then add a child `Path2D <class_Path2D>`{.interpreted-text
role="ref"} node named `MobPath`. When you select `Path2D`, you will see
...
public void NewGame() { [score]{#score} = 0;
...
Signals tab of the Node dock, then double-click the `timeout()` signal
in the list. This will open a new signal connection dialog. The default
settings in this dialog should be fine, so select **Connect** to create
a new signal connection.

Once all three timers have this set up, you should be able to see each
timer have a Signal connection for their respective `timeout()` signal,
showing in green, within their respective Signals tabs:
...
func [on_score_timer_timeout]{#on_score_timer_timeout}():

:   score += 1

func [on_start_timer_timeout]{#on_start_timer_timeout}():
...
connection window. private void OnScoreTimerTimeout() {
[score]{#score}++; }
...
func [on_mob_timer_timeout]{#on_mob_timer_timeout}():
...
:   \# Create a new instance of the Mob scene. var mob =
    ...
    \# Set the mob\'s position to the random location. mob.position =
...
public override void [Ready]{#ready}() { NewGame(); }
::::
...
right click `main.tscn` in the FileSystem dock and select \"Set As Main
Scene\".
::::
