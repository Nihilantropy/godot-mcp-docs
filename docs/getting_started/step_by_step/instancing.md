# Creating instances {#doc_instancing}
...
instance scenes from code, see
`doc_nodes_and_scene_instances`{.interpreted-text role="ref"}.

Godot\'s approach to *instancing* described below should not be confused
with hardware instancing that can be used to render large amounts of
similar objects quickly. See `doc_using_multimesh`{.interpreted-text
role="ref"} instead.
::::
...
In the Scene dock on the left, select the Ball node. Then, in the
Inspector on the right, click on the PhysicsMaterial property to expand
it.
...
player\'s perspective. The arrows point towards the instantiator of each
scene.
