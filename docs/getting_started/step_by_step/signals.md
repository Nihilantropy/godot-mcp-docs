...
- **GDScript**: Classes (nodes) use PascalCase, variables and functions
  use snake_case, and constants use ALL_CAPS (See
  `doc_gdscript_styleguide`{.interpreted-text role="ref"}).
- **C#**: Classes, export variables and methods use PascalCase, private
  fields use [camelCase]{#camelcase}, local variables and parameters use
...
code generation might not work. In this case, you need to connect the
...
[angularSpeed]{#angularspeed} \* (float)delta; var velocity =
Vector2.Up.Rotated(Rotation) \* [speed]{#speed}; Position += velocity \*
(float)delta; }
::::
...
public partial class MySprite2D : Sprite2D { private float
[speed]{#speed} = 400; private float [angularSpeed]{#angularspeed} =
Mathf.Pi;
...
Run the current scene by pressing `F6`{.interpreted-text role="kbd"}
(`Cmd + R`{.interpreted-text role="kbd"} on macOS), and click the button
to see the sprite start and stop.
...
public override void [Ready]{#ready}() { var timer =
GetNode\<Timer\>(\"Timer\"); }
::::
...
:   var timer = get_node(\"Timer\")
    timer.timeout.connect([on_timer_timeout]{#on_timer_timeout})
...
Here, it\'ll be \"\_on_timer_timeout\" for GDScript and OnTimerTimeout()
for C#.
::::
...
func [on_timer_timeout]{#on_timer_timeout}():
...
public void TakeDamage(int amount) { [health]{#health} -= amount;

> if ([health]{#health} \<= 0) { EmitSignal(SignalName.HealthDepleted);
> }
...
public void TakeDamage(int amount) { int oldHealth = [health]{#health};
[health]{#health} -= amount; EmitSignal(SignalName.HealthChanged,
