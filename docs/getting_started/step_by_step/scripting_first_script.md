# Creating your first script {#doc_scripting_first_script}

In this lesson, you will code your first script to make the Godot icon
turn in circles. As we mentioned `in the introduction
<doc_introduction_learning_programming>`{.interpreted-text role="ref"},
...
`doc_gdscript`{.interpreted-text role="ref"} section. To learn more
about C#, head to the `doc_c_sharp`{.interpreted-text role="ref"}
section.
:::
...
will implicitly extend `RefCounted <class_RefCounted>`{.interpreted-text
role="ref"}, which Godot uses to manage your application\'s memory.
::::
...
private int [speed]{#speed} = 400; private float
[angularSpeed]{#angularspeed} = Mathf.Pi;
::::
...
[angularSpeed]{#angularspeed} \* (float)delta; }
::::
...
In the code editor, you can `Ctrl + Click`{.interpreted-text role="kbd"}
(`Cmd + Click`{.interpreted-text role="kbd"} on macOS) on any built-in
property or function like `position`, `rotation`, or `_process` to open
the corresponding documentation in a new tab.
::::
...
var velocity = Vector2.Up.Rotated(Rotation) \* [speed]{#speed};
...
or the floor. In `doc_your_first_2d_game`{.interpreted-text role="ref"},
you will learn another approach to moving objects while detecting
collisions.
::::
...
public partial class MySprite2D : Sprite2D { private int [speed]{#speed}
= 400; private float [angularSpeed]{#angularspeed} = Mathf.Pi;

> public override void [Process]{#process}(double delta) { Rotation +=
> [angularSpeed]{#angularspeed} \* (float)delta; var velocity =
> Vector2.Up.Rotated(Rotation) \* [speed]{#speed};
>
> > Position += velocity \* (float)delta;
>
> }
