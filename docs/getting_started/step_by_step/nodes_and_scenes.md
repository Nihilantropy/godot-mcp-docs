...
add a root node quickly. `2D Scene`{.interpreted-text role="button"}
adds a `Node2D <class_Node2D>`{.interpreted-text role="ref"} node,
`3D Scene`{.interpreted-text role="button"} adds a
`Node3D <class_Node3D>`{.interpreted-text role="ref"} node, and
`User Interface`{.interpreted-text role="button"} adds a
`Control <class_Control>`{.interpreted-text role="ref"} node. These
presets are here for convenience; they are not mandatory.
`Other Node`{.interpreted-text role="button"} lets you select any node
to be the root node. In an empty scene, `Other Node`{.interpreted-text
role="button"} is equivalent to pressing the
`Add Child Node`{.interpreted-text role="button"} button at the top-left
of the Scene dock, which usually adds a new node as a child of the
...
Press the `Add Child Node`{.interpreted-text role="button"} button or
`Other Node`{.interpreted-text role="button"} to create a root node.

![image](img/nodes_and_scenes_02_scene_dock.webp)

The `Create New Node`{.interpreted-text role="ui"} dialog opens, showing
the long list of available nodes.
...
Click on the Label node to select it and click the
`Create`{.interpreted-text role="button"} button at the bottom of the
window.
...
The next step is to change the Label\'s `Text`{.interpreted-text
role="inspector"} property. Let\'s change it to \"Hello World\".

Head to the Inspector dock on the right of the viewport. Click inside
the field below the `Text`{.interpreted-text role="inspector"} property
...
Everything\'s ready to run the scene! Press the
`Run Current Scene`{.interpreted-text role="button"} button in the
...
Click the `Save`{.interpreted-text role="button"} button in the file
browser to save it as `label.tscn`.
...
The `Save Scene As`{.interpreted-text role="ui"} dialog, like other file
dialogs in the editor, only allows you to save files inside the project.
The `res://` path at the top of the window represents the project\'s
root directory and stands for \"resource path\". For more information
about file paths in Godot, see `doc_filesystem`{.interpreted-text
role="ref"}.
::::
...
To run our test scene, we used the `Run Current Scene`{.interpreted-text
role="button"} button. Another button next to it,
`Run Project`{.interpreted-text role="button"}, allows you to set and
run the project\'s **main scene**. You can also press
`F5`{.interpreted-text role="kbd"} (`Cmd + B`{.interpreted-text
role="kbd"} on macOS) to do so.
...
*current scene*. If you encounter unexpected behavior, check to ensure
you are running the correct scene.
::::
...
Click the `Select`{.interpreted-text role="button"} button, and in the
file dialog that appears, double click on `label.tscn`.
...
`Project > Project Settings`{.interpreted-text role="menu"} window to do
so. For more information, see `doc_project_settings`{.interpreted-text
role="ref"}.
::::
