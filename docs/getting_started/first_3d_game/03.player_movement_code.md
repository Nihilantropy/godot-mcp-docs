# Moving the player with code {#doc_first_3d_game_player_movement}
...
Here, instead of `_process()`, we\'re going to make all calculations
using the `_physics_process()` virtual function. It\'s designed
specifically for physics-related code like moving a kinematic or rigid
body. It updates the node using fixed time intervals.
...
> // Ground velocity [targetVelocity.X]{#targetvelocity.x} = direction.X
> \* Speed; [targetVelocity.Z]{#targetvelocity.z} = direction.Z \*
> Speed;
>
> // Vertical velocity if (!IsOnFloor()) // If in the air, fall towards
> the floor. Literally gravity { [targetVelocity.Y]{#targetvelocity.y}
> -= FallAcceleration \* (float)delta; }
>
> // Moving the character Velocity = [targetVelocity]{#targetvelocity};
> MoveAndSlide();
...
> > towards the floor. Literally gravity {
