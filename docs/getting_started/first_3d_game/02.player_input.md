...
Click the **Other Node** button and select the `CharacterBody3D` node
type to create a
...
Rename the `CharacterBody3D <class_CharacterBody3D>`{.interpreted-text
...
child of `Player`. Select the `Player` node in the **Scene** tree and
click the \"\*\*+\*\*\" button to add a child node. Rename it to
`Pivot`.
...
by clicking the **eye icon** next to the `Character` or the `Pivot`
nodes.
...
## Creating input actions {#doc_first_3d_game_input_actions}
...
you enable the **Show Built-in Actions** toggle. We could use these
here, but instead we\'re defining our own to support gamepads. Leave
**Show Built-in Actions** disabled.
...
To bind a key or button to an action, click the \"\*\*+\*\*\" button to
its right. Do this for `move_left`. Press the left arrow key and click
**OK**.
...
\"\*\*+\*\*\" button again but this time, select the input within the
input tree yourself. Select the negative X axis of the left joystick
under **Joypad Axes**.
