# Character animation {#doc_first_3d_game_character_animation}
...
If you don\'t see the properties in the *Inspector* panel, first click
...
Vector3(Mathf.Pi / 6.0f \* Velocity.Y / JumpImpulse, pivot.Rotation.Y,
pivot.Rotation.Z); }
:::
::::
...
**New Library**. You should see the message \"Global library will be
...
:   \# We create a local variable to store the input direction var
    ...
    Vector3.ZERO: direction = direction.normalized() \# Setting the
    basis property will affect the rotation of the node. \$Pivot.basis =
    Basis.looking_at(direction) \$AnimationPlayer.speed_scale = 4 else:
    \$AnimationPlayer.speed_scale = 1
...
func [on_mob_detector_body_entered]{#on_mob_detector_body_entered}(body):
...
>
> private Vector3 [targetVelocity]{#targetvelocity} = Vector3.Zero;
>
> public override void [PhysicsProcess]{#physicsprocess}(double delta) {
> // We create a local variable to store the input direction. var
> direction = Vector3.Zero;
...
> > // Prevent diagonal movement being very fast. if (direction !=
> > Vector3.Zero) { direction = direction.Normalized(); // Setting the
> > basis property will affect the rotation of the node.
> > GetNode\<Node3D\>(\"Pivot\").Basis = Basis.LookingAt(direction);
> > GetNode\<AnimationPlayer\>(\"AnimationPlayer\").SpeedScale = 4; }
> > else { GetNode\<AnimationPlayer\>(\"AnimationPlayer\").SpeedScale =
> > 1; }
> >
> > // Ground velocity [targetVelocity.X]{#targetvelocity.x} =
> > direction.X \* Speed; [targetVelocity.Z]{#targetvelocity.z} =
> > direction.Z \* Speed;
> >
> > // Vertical velocity if (!IsOnFloor()) {
> > [targetVelocity.Y]{#targetvelocity.y} -= FallAcceleration \*
> > (float)delta; }
> >
> > // Jumping. if (IsOnFloor() && Input.IsActionJustPressed(\"jump\"))
> > { [targetVelocity.Y]{#targetvelocity.y} += JumpImpulse; }
> >
...
> > > squash it and bounce. mob.Squash();
> > > [targetVelocity.Y]{#targetvelocity.y} = BounceImpulse; // Prevent
> > > further duplicate calls. break; } }
> >
> > }
> >
> > // Moving the character Velocity =
> > [targetVelocity]{#targetvelocity}; MoveAndSlide();
> >
> > var pivot = GetNode\<Node3D\>(\"Pivot\"); pivot.Rotation = new
...
func [on_visible_on_screen_notifier_3d_screen_exited]{#on_visible_on_screen_notifier_3d_screen_exited}():
