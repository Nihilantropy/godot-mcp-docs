...
A ray in 2D space, used to find the first collision object it
intersects.
...
role="ref"} and
`add_exception()<class_RayCast2D_method_add_exception>`{.interpreted-text
role="ref"}.
...
`TileMapLayer<class_TileMapLayer>`{.interpreted-text role="ref"}, the
