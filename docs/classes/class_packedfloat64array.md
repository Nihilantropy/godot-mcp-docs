...
`float<class_float>`{.interpreted-text role="ref"})
`🔗<class_PackedFloat64Array_method_erase>`{.interpreted-text
role="ref"}
...
`remove_at()<class_PackedFloat64Array_method_remove_at>`{.interpreted-text
role="ref"} instead.
...
Returns the 64-bit float at the given `index` in the array. If `index`
out-of-bounds or negative, this method fails and returns `0.0`.
...
`size()<class_PackedFloat64Array_method_size>`{.interpreted-text
