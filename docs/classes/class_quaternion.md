...
are faster than their `Basis<class_Basis>`{.interpreted-text role="ref"}
counterparts.
...
`Quaternion<class_Quaternion_constructor_Quaternion>`{.interpreted-text
...
Constructs a **Quaternion** identical to
`IDENTITY<class_Quaternion_constant_IDENTITY>`{.interpreted-text
role="ref"}.

\*\*Note:\*\* In C#, this constructs a **Quaternion** with all of its
components set to `0.0`.
...
`length()<class_Quaternion_method_length>`{.interpreted-text
role="ref"}, so prefer it if you only need to compare quaternion
lengths.
