...
role="ref"}, `FoldableGroup<class_FoldableGroup>`{.interpreted-text
...
:::: {#enum_Resource_DeepDuplicateMode}
::: rst-class
...
`🔗<enum_Resource_DeepDuplicateMode>`{.interpreted-text role="ref"}

:::: {#class_Resource_constant_DEEP_DUPLICATE_NONE}
::: rst-class
classref-enumeration-constant
:::
::::

`DeepDuplicateMode<enum_Resource_DeepDuplicateMode>`{.interpreted-text
role="ref"} **DEEP_DUPLICATE_NONE** = `0`

No subresorces at all are duplicated. This is useful even in a deep
duplication to have all the arrays and dictionaries duplicated but still
pointing to the original resources.

:::: {#class_Resource_constant_DEEP_DUPLICATE_INTERNAL}
::: rst-class
classref-enumeration-constant
:::
::::
...
`DeepDuplicateMode<enum_Resource_DeepDuplicateMode>`{.interpreted-text
role="ref"} **DEEP_DUPLICATE_INTERNAL** = `1`

Only subresources without a path or with a scene-local path will be
duplicated.

:::: {#class_Resource_constant_DEEP_DUPLICATE_ALL}
::: rst-class
classref-enumeration-constant
:::
::::
...
`DeepDuplicateMode<enum_Resource_DeepDuplicateMode>`{.interpreted-text
role="ref"} **DEEP_DUPLICATE_ALL** = `2`

Every subresource found will be duplicated, even if it has a non-local
path. In other words, even potentially big resources stored separately
will be duplicated.
...
A unique identifier relative to the this resource\'s scene. If left
...
role="ref"}, this method must be implemented to clear them.
...
Override this method to execute additional logic after
`set_path_cache()<class_Resource_method_set_path_cache>`{.interpreted-text
role="ref"} is called on this object.
...
If `deep` is `false`, a **shallow** copy is returned: nested
`Array<class_Array>`{.interpreted-text role="ref"},
`Dictionary<class_Dictionary>`{.interpreted-text role="ref"}, and
**Resource** properties are not duplicated and are shared with the
original resource.

If `deep` is `true`, a **deep** copy is returned: all nested arrays,
dictionaries, and packed arrays are also duplicated (recursively). Any
**Resource** found inside will only be duplicated if it\'s local, like
`DEEP_DUPLICATE_INTERNAL<class_Resource_constant_DEEP_DUPLICATE_INTERNAL>`{.interpreted-text
role="ref"} used with
`duplicate_deep()<class_Resource_method_duplicate_deep>`{.interpreted-text
role="ref"}.
...
- Subresource properties with the
  ...
  role="ref"} flag are always duplicated (recursively or not, depending
  on `deep`).
- Subresource properties with the
...
\*\*Note:\*\* When duplicating with `deep` set to `true`, each resource
found, including the one on which this method is called, will be only
duplicated once and referenced as many times as needed in the duplicate.
For instance, if you are duplicating resource A that happens to have
resource B referenced twice, you\'ll get a new resource A\' referencing
a new resource B\' twice.
...
:::: {#class_Resource_method_duplicate_deep}
::: rst-class
classref-method
:::
::::
...
`const (This method has no side effects. It doesn't modify any of the instance's member variables.)`{.interpreted-text
role="abbr"}
`🔗<class_Resource_method_duplicate_deep>`{.interpreted-text role="ref"}

Duplicates this resource, deeply, like
`duplicate()<class_Resource_method_duplicate>`{.interpreted-text
role="ref"}`(true)`, with extra control over how subresources are
handled.

`deep_subresources_mode` must be one of the values from
`DeepDuplicateMode<enum_Resource_DeepDuplicateMode>`{.interpreted-text
role="ref"}.
...
From the internal cache for scene-unique IDs, returns the ID of this
resource for the scene at `path`. If there is no entry, an empty string
is returned. Useful to keep scene-unique IDs the same when implementing
a VCS-friendly custom resource format by extending
...
Returns `true` if the resource is saved on disk as a part of another
resource\'s file.
...
Makes the resource clear its non-exported properties. See also
`_reset_state()<class_Resource_private_method__reset_state>`{.interpreted-text
role="ref"}. Useful when implementing a custom resource format by
extending
`ResourceFormatLoader<class_ResourceFormatLoader>`{.interpreted-text
role="ref"} and
`ResourceFormatSaver<class_ResourceFormatSaver>`{.interpreted-text
...
In the internal cache for scene-unique IDs, sets the ID of this resource
to `id` for the scene at `path`. If `id` is empty, the cache entry for
`path` is cleared. Useful to keep scene-unique IDs the same when
implementing a VCS-friendly custom resource format by extending
`ResourceFormatLoader<class_ResourceFormatLoader>`{.interpreted-text
role="ref"} and
`ResourceFormatSaver<class_ResourceFormatSaver>`{.interpreted-text
role="ref"}.
...
cache. Useful for handling
`CacheMode<enum_ResourceFormatLoader_CacheMode>`{.interpreted-text
role="ref"} values when implementing a custom resource format by
extending
`ResourceFormatLoader<class_ResourceFormatLoader>`{.interpreted-text
role="ref"} and
`ResourceFormatSaver<class_ResourceFormatSaver>`{.interpreted-text
