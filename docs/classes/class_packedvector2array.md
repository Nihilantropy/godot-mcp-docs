...
`Vector2<class_Vector2>`{.interpreted-text role="ref"})
`🔗<class_PackedVector2Array_method_erase>`{.interpreted-text
role="ref"}
...
`remove_at()<class_PackedVector2Array_method_remove_at>`{.interpreted-text
role="ref"} instead.
...
this method fails and returns `Vector2(0, 0)`.
...
`size()<class_PackedVector2Array_method_size>`{.interpreted-text
