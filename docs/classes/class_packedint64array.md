...
compared to a typed array of the same type (e.g. **PackedInt64Array**
...
`Array.map()<class_Array_method_map>`{.interpreted-text role="ref"}.
Typed arrays are in turn faster to iterate on and modify than untyped
arrays.
...
:::: {#class_PackedInt64Array_method_erase}
::: rst-class
classref-method
:::
::::
...
`int<class_int>`{.interpreted-text role="ref"})
`🔗<class_PackedInt64Array_method_erase>`{.interpreted-text role="ref"}
...
`remove_at()<class_PackedInt64Array_method_remove_at>`{.interpreted-text
role="ref"} instead.
...
Returns the 64-bit integer at the given `index` in the array. If `index`
...
`size()<class_PackedInt64Array_method_size>`{.interpreted-text
...
`PackedByteArray<class_PackedByteArray>`{.interpreted-text role="ref"},
where each element has been encoded as 8 bytes.
