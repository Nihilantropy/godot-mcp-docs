...
func [input]{#input}(input_event):

:   

    if input_event is InputEventMIDI:

    :   [print_midi_info]{#print_midi_info}(input_event)

func [print_midi_info]{#print_midi_info}(midi_event):
...
public override void [Ready]{#ready}() { OS.OpenMidiInputs();
GD.Print(OS.GetConnectedMidiInputs()); }

public override void [Input]{#input}(InputEvent inputEvent) { if
(inputEvent is InputEventMidi midiEvent) { PrintMIDIInfo(midiEvent); } }
...
when calling
`OS.open_midi_inputs()<class_OS_method_open_midi_inputs>`{.interpreted-text
role="ref"}. MIDI input will not work until the user accepts the
permission request.
