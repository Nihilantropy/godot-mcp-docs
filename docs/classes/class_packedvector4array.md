...
compared to a typed array of the same type (e.g. **PackedVector4Array**
versus `Array[Vector4]`). Also, packed arrays consume less memory. As a
...
`Vector4<class_Vector4>`{.interpreted-text role="ref"})
`🔗<class_PackedVector4Array_method_erase>`{.interpreted-text
role="ref"}
...
`remove_at()<class_PackedVector4Array_method_remove_at>`{.interpreted-text
role="ref"} instead.
...
the given `index` in the array. If `index` out-of-bounds or negative,
this method fails and returns `Vector4(0, 0, 0, 0)`.
...
`size()<class_PackedVector4Array_method_size>`{.interpreted-text
