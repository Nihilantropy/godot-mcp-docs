# Upgrading from Godot 3 to Godot 4 {#doc_upgrading_to_godot_4}
...
4.0](https://godotengine.org/article/godot-4-0-sets-sail), upgrading
...
pages documenting new features in Godot 4.0, and
`doc_list_of_features`{.interpreted-text role="ref"} for a list of all
features in Godot.

### Disadvantages of upgrading

If you don\'t *need* any features present in Godot 4.x, you may want to
...
- Godot 4 does not and will not have support for GLES2 rendering. (There
  is still support for GLES3 rendering using the new Compatibility
  renderer, which means that devices without Vulkan support can still
  run Godot 4.)
...
- By default, rendering in 2D is no longer performed in HDR, which means
  ...
  role="ref"}.
- While rendering still happens in HDR in 3D when using the Forward+ or
  Mobile renderers, Viewports cannot return HDR data anymore. This is
...
If you still want large files to be converted by the project upgrade
...
dialog doesn\'t offer any way to undo a replacement. Use version control
to commit your upgrade work regularly. Command line tools such as
[sd](https://github.com/chmln/sd) can also be used if you need something
more flexible than the editor\'s Replace in Files dialog.
...
- AnimationPlayer\'s `add_animation()` is now `add_animation_library()`
  and now uses an `class_AnimationLibrary`{.interpreted-text
  role="ref"}.
...
- MultiplayerAPI\'s `is_refusing_new_network_connections()` is now
...
project settings. This was done to make runtime quality adjustments
...
See `doc_shading_language`{.interpreted-text role="ref"} for more
information.
...
- `class_AnimatedTexture`{.interpreted-text role="ref"}\'s `fps`
  property was replaced by `speed_scale`, which works the same as
...
the project converter doesn\'t support updating existing setups:
  ...
  ClippedCamera         Camera2D or      Camera\'s pyramid shape was moved to :ref:\'class_Camera3D\'.
                        Camera3D         

  InterpolatedCamera    Camera2D or      
                        Camera3D         

  Navigation2D          Node2D           Replaced by
                                         `other 2D Navigation nodes <doc_navigation_overview_2d>`{.interpreted-text
                                         role="ref"}.

  Navigation3D          Node3D           Replaced by
                                         `other 3D Navigation nodes <doc_navigation_overview_3d>`{.interpreted-text
                                         role="ref"}.

  OpenSimplexNoise      FastNoiseLite    Has different parameters and more noise types such as cellular. No support
                                         for 4D noise as it\'s absent from the FastNoiseLite library.

  ToolButton            Button           ToolButton was Button with the **Flat** property enabled by default.

  YSort                 Node2D or        CanvasItem has a new **Y Sort Enabled** property in 4.0.
                        Control          

  ProximityGroup        Node3D           `class_VisibleOnScreenNotifier3D`{.interpreted-text role="ref"} can act as a
                                         replacement.

  Portal                Node3D           Portal and room occlusion culling was replaced by raster
                                         `occlusion culling <doc_occlusion_culling>`{.interpreted-text role="ref"}
...
Editor settings whose name or category has changed won\'t carry over to
Godot 4.0; you will have to set their values again.
::::

## Updating version control settings

Godot 3.x and 4.x have entirely different lists of files and folders
that should be ignored by your
`version control system<doc_version_control_systems>`{.interpreted-text
role="ref"}.
