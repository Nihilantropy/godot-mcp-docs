# Customizing the mouse cursor {#doc_custom_mouse_cursor}
...
but this will add at least one frame of latency compared to a
...
Open the **Project Settings** and go to **Display \> Mouse Cursor**. You
will see the settings
`Custom Image <class_ProjectSettings_property_display/mouse_cursor/custom_image>`{.interpreted-text
role="ref"},
`Custom Image Hotspot <class_ProjectSettings_property_display/mouse_cursor/custom_image_hotspot>`{.interpreted-text
role="ref"}, and
`Tooltip Position Offset <class_ProjectSettings_property_display/mouse_cursor/tooltip_position_offset>`{.interpreted-text
role="ref"}.

![image](img/cursor_project_settings.webp)

**Custom Image** is the desired image that you would like to set as the
mouse cursor. **Custom Hotspot** is the point in the image that you
would like to use as the cursor\'s detection point.
...
public partial class MyNode : Node { public override void
[Ready]{#ready}() { // Load the custom images for the mouse cursor. var
arrow = ResourceLoader.Load(\"res://arrow.png\"); var beam =
...
`Input.CursorShape <enum_Input_CursorShape>`{.interpreted-text
role="ref"} enum. Which ones you want to use depends on your use case.
