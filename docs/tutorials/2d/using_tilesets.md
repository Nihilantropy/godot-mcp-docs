...
It\'s recommended to only use scene tiles when necessary. To draw
...
We\'ve now successfully created a basic TileSet. We could start using it
in the TileMapLayer node now, but it currently lacks any form of
collision detection. This means the player and other objects could walk
straight through the floor or walls.
...
Godot offers *terrains* to perform this kind of tile connection
