...
`Parallax2D<class_parallax2d>`{.interpreted-text role="ref"}, which is
recommended to use over the
`ParallaxLayer<class_parallaxlayer>`{.interpreted-text role="ref"} and
...
so you can use one or many nodes to make up each layer. To begin, place
each node or nodes you want to have scroll independently as a child of
their own parallax node. Make sure that the top left of the textures
used are at the `(0, 0)` crossing, like in the image below. See the
section on `positioning <doc_2d_parallax_positioning>`{.interpreted-text
role="ref"} for why this is important.

![image](img/2d_parallax_size_viewport.webp)

The scene above uses one prepared texture for the higher clouds in a
`Sprite2D <class_sprite2d>`{.interpreted-text role="ref"}, but you could
...
::: {.video alt="A scene with five layers scrolling at different speeds" autoplay="" loop="" muted="" align="default"}
...
than your textures. In **Project Settings \> Display \> Window**, change
the
`Viewport Width<class_ProjectSettings_property_display/window/size/viewport_width>`{.interpreted-text
role="ref"} and
`Viewport Height<class_ProjectSettings_property_display/window/size/viewport_height>`{.interpreted-text
role="ref"} settings to match your background.
...
### Poor positioning {#doc_2d_parallax_positioning}
