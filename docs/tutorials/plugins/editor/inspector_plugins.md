...
MyInspectorPlugin [plugin]{#plugin};

> public override void [EnterTree]{#entertree}() { [plugin]{#plugin} =
> new MyInspectorPlugin(); AddInspectorPlugin([plugin]{#plugin}); }
>
> public override void [ExitTree]{#exittree}() {
> RemoveInspectorPlugin([plugin]{#plugin}); }
...
func [can_handle]{#can_handle}(object):

:   \# We support all objects in this example. return true

func [parse_property]{#parse_property}(object, type, name, hint_type, hint_string, usage_flags, wide):
...
override bool [CanHandle]{#canhandle}(GodotObject \@object) { // We
support all objects in this example. return true; }

> 
>
> public override bool [ParseProperty]{#parseproperty}(GodotObject \@object, Variant.Type type,
>
...
:   \# Add the control as a direct child of EditorProperty node.
    ...
    property_control.pressed.connect([on_button_pressed]{#on_button_pressed})

func [on_button_pressed]{#on_button_pressed}():
...
func [update_property]{#update_property}():
...
control for editing the property. private Button
[propertyControl]{#propertycontrol} = new Button(); // An internal value
of the property. private int [currentValue]{#currentvalue} = 0; // A
guard against internal changes when the property is updated. private
bool [updating]{#updating} = false;

> public RandomIntEditor() { // Add the control as a direct child of
> EditorProperty node. AddChild([propertyControl]{#propertycontrol}); //
> Make sure the control is able to retain the focus.
> AddFocusable([propertyControl]{#propertycontrol}); // Setup the
> initial state and connect to the signal to track changes.
> RefreshControlText();
> [propertyControl.Pressed]{#propertycontrol.pressed} +=
> OnButtonPressed; }
>
> private void OnButtonPressed() { // Ignore the signal if the property
> is currently being updated. if ([updating]{#updating}) { return; }
>
> > // Generate a new random integer between 0 and 99.
> > [currentValue]{#currentvalue} = (int)GD.Randi() % 100;
> > RefreshControlText(); EmitChanged(GetEditedProperty(),
> > [currentValue]{#currentvalue});
>
> }
>
> public override void [UpdateProperty]{#updateproperty}() { // Read the
> current value from the property. var newValue =
> (int)GetEditedObject().Get(GetEditedProperty()); if (newValue ==
> [currentValue]{#currentvalue}) { return; }
>
> > // Update the control with the new value. [updating]{#updating} =
> > true; [currentValue]{#currentvalue} = newValue;
> > RefreshControlText(); [updating]{#updating} = false;
>
> }
>
> private void RefreshControlText() {
> [propertyControl.Text]{#propertycontrol.text} = \$\"Value:
> {[currentValue]{#currentvalue}}\"; }
