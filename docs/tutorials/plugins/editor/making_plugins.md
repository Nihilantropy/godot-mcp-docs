...
> override void [EnterTree]{#entertree}() { // Initialization of the
> plugin goes here. }
>
> > public override void [ExitTree]{#exittree}() { // Clean-up of the
> > plugin goes here. }
>
> } #endif
...
[EnterTree]{#entertree}() { Pressed += Clicked; }
...
\[Tool\] public partial class CustomNode : EditorPlugin { public
override void [EnterTree]{#entertree}() { // Initialization of the
plugin goes here. // Add the new type with a name, a parent type, a
script and an icon. var script =
GD.Load\<Script\>(\"res://addons/MyCustomNode/MyButton.cs\"); var
texture = GD.Load\<Texture2D\>(\"res://addons/MyCustomNode/Icon.png\");
AddCustomType(\"MyButton\", \"Button\", script, texture); }

> public override void [ExitTree]{#exittree}() { // Clean-up of the
> plugin goes here. // Always remember to remove it from the engine when
...
> public override void [EnterTree]{#entertree}() { [dock]{#dock} =
> GD.Load\<PackedScene\>(\"res://addons/MyCustomDock/MyDock.tscn\").Instantiate\<Control\>();
> AddControlToDock(DockSlot.LeftUl, [dock]{#dock}); }
>
> public override void [ExitTree]{#exittree}() { // Clean-up of the
> plugin goes here. // Remove the dock.
> RemoveControlFromDocks([dock]{#dock}); // Erase the control from the
> memory. [dock.Free]{#dock.free}(); }
...
> public override void [EnablePlugin]{#enableplugin}() { // The autoload
> can be a scene or script file. AddAutoloadSingleton(AutoloadName,
> \"res://addons/MyAddon/SomeAutoload.tscn\"); }
>
> public override void [DisablePlugin]{#disableplugin}() {
...
Often a plugin adds multiple things, for example a custom node and a
panel. In those cases it might be easier to have a separate plugin
script for each of those features. Sub-plugins can be used for this.
...
\# The main plugin is located at <res://addons/my_plugin/> const
