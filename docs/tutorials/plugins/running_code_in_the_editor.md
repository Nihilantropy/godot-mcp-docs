...
public override void [Process]{#process}(double delta) { if
...
## Important information {#doc_running_code_in_the_editor_important_information}

The general rule is that **any other GDScript that your tool script uses
must \*also\* be a tool**. The editor is not able to construct instances
from GDScript files without `@tool`, which means you cannot call methods
or reference member variables from them otherwise. However, since static
methods, constants and enums can be used without creating an instance,
it is possible to call them or reference them from a `@tool` script onto
other non-tool scripts. One exception to this are
`static variables <doc_gdscript_basics_static_variables>`{.interpreted-text
role="ref"}. If you try to read a static variable\'s value in a script
that does not have `@tool`, it will always return `null` but won\'t
print a warning or error when doing so. This restriction does not apply
to static methods, which can be called regardless of whether the target
script is in tool mode.
...
For example, in the next section when we remove the script, the node
will keep its rotation. Be careful to avoid making unwanted
modifications. Consider setting up
`version control <doc_version_control_systems>`{.interpreted-text
role="ref"} to avoid losing work in case you make a mistake.

Using the debugger and breakpoints on tool scripts is not currently
supported. Breakpoints placed in the script editor or using the
`breakpoint` keyword are ignored. You can use print statements to
...
[Process]{#process}(double delta) { Rotation += Mathf.Pi \*
(float)delta; } }
::::
...
You may need to restart the editor. This is a known bug found in all
Godot 4 versions:
[GH-66381](https://github.com/godotengine/godot/issues/66381).
::::
...
> \[Export\] public float Speed { get =\> [speed]{#speed}; set { //
> Update speed and reset the rotation. [speed]{#speed} = value; Rotation
> = 0; } }
>
> public override void [Process]{#process}(double delta) { Rotation +=
> Mathf.Pi \* (float)delta \* [speed]{#speed}; }
...
:   
    ...
    :   resource = new_resource [on_resource_set]{#on_resource_set}()
...
func [on_resource_set]{#on_resource_set}(): print(\"My resource was
set!\")
...
> \[Export\] public MyResource Resource { get =\> [resource]{#resource};
> set { [resource]{#resource} = value; OnResourceSet(); } }
...
> \[Export\] public float Property { get =\> [property]{#property}; set
> { [property]{#property} = value; // Emit a signal when the property is
> changed. EmitChanged(); } }
...
:   
    ...
    :   resource = new_resource \# Connect the changed signal as soon as
        ...
        resource.changed.connect([on_resource_changed]{#on_resource_changed})

func [on_resource_changed]{#on_resource_changed}():
...
> \[Export\] public MyResource Resource { get =\> [resource]{#resource};
> set { [resource]{#resource} = value; // Connect the changed signal as
> soon as a new resource is being added.
> [resource.Changed]{#resource.changed} += OnResourceChanged; } }
...
:   
    ...
    :   \# Disconnect the signal if the previous resource was not null.
        if resource != null:
        resource.changed.disconnect([on_resource_changed]{#on_resource_changed})
...
\[Export\] public MyResource Resource { get =\> [resource]{#resource};
set { // Disconnect the signal if the previous resource was not null. if
([resource]{#resource} != null) { [resource.Changed]{#resource.changed}
-= OnResourceChanged; } [resource]{#resource} = value;
[resource.Changed]{#resource.changed} += OnResourceChanged; } }
::::
...
func [get_configuration_warnings]{#get_configuration_warnings}():
...
## Running one-off scripts using EditorScript {#doc_running_code_in_the_editor_editorscript}
...
EditorScripts can only be run from the Godot script editor. If you are
using an external editor, open the script inside the Godot script editor
to run it.
::::
...
public override void [Ready]{#ready}() { var node = new Node3D();
...
:   \# [parent]{.title-ref} could be any node in the scene. var parent =
...
public override void [Run]{#run}() { // [parent]{.title-ref} could be
any node in the scene. var parent = GetScene().GetNode(\"Parent\"); var
node = new Node3D(); parent.AddChild(node);
