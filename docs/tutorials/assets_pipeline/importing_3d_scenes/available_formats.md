# Available 3D formats {#doc_importing_3d_scenes_available_formats}
...
- FBX, supported via the [ufbx](https://github.com/ufbx/ufbx) library.
  ...
  proprietary FBX SDK, so we recommend using the default ufbx method or
...
\"morph targets\"), your glTF export setting **Data \> Armature \>
Export Deformation Bones Only** needs to be configured to **Enabled**.
...
The editor can directly import `.blend` files by calling
[Blender](https://www.blender.org/)\'s glTF export functionality in a
transparent manner.
...
case, configure the path to the Blender executable in the Editor
Settings (**Filesystem \> Import \> Blender \> Blender Path**).
...
For complex scenes or scenes that contain animations it is highly
recommend to use glTF instead.
...
those vertex colors, but they will not be displayed on the material
unless you enable **Vertex Color \> Use As Albedo** on the material.
