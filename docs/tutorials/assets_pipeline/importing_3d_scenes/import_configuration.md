# Import configuration {#doc_importing_3d_scenes_import_configuration}
...
### Using the Import dock {#doc_importing_3d_scenes_using_the_import_dock}
...
- **Generate \> LODs:** Per-mesh override for the **Meshes \> Generate
...
slices. For this to work, the model must have only one animation that is
named `default`. To create slices, change the slice amount to something
greater than zero. You can then name a slice, specify which frames it
