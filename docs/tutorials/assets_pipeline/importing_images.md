# Importing images {#doc_importing_images}
...
- SVG (`.svg`)
  ...
  limited](https://www.thorvg.org/about#:~:text=certain%20features%20remain%20unsupported%20within%20the%20current%20framework);
...
- **CubemapArray:** Import the texture as a collection of 6-sided
  ...
  can only be displayed when using the Forward+ or Mobile renderers, not
...
- **Texture3D:** Import the image as a 3-dimensional texture. This is
  *not* a 2D texture applied onto a 3D surface. Texture3D is similar to
...
![bad](img/bad.png) = worst):
  ...
  ----------------- ----------------------------- ----------------------------- ----------------------------- ----------------------------- -----------------------------
  **Description**   Stored as Lossless WebP / PNG Stored as Lossy WebP          Stored as S3TC, BPTC or ETC2  Stored as raw pixels          Transcoded to VRAM Compressed
                                                                                depending on platform                                       format

  **Size on disk**  ![regular](img/regular.png)   ![good](img/good.png) Very    ![regular](img/regular.png)   ![bad](img/bad.png) Large     ![good](img/good.png) Very
  ...
  **Performance**   ![regular](img/regular.png)   ![regular](img/regular.png)   ![good](img/good.png) Fast    ![regular](img/regular.png)   ![good](img/good.png) Fast
                    Normal                        Normal                                                      Normal                        

  **Quality loss**  ![good](img/good.png) None    ![regular](img/regular.png)   ![bad](img/bad.png) Moderate  ![good](img/good.png) None    ![bad](img/bad.png) Moderate
                                                  Slight                                                                                    

  **Load time**     ![bad](img/bad.png) Slow      ![bad](img/bad.png) Slow      ![good](img/good.png) Fast    ![regular](img/regular.png)   ![regular](img/regular.png)
                                                                                                              Normal                        Normal
  -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Estimated memory usage for a single RGBA8 texture with mipmaps enabled:
  ...
  **512×512**     ![good](img/good.png) 1.33    ![good](img/good.png) 1.33    ![good](img/good.png) 341 KiB ![good](img/good.png) 1.33    ![good](img/good.png) 341 KiB
                  MiB                           MiB                                                         MiB                           

  **1024×1024**   ![regular](img/regular.png)   ![regular](img/regular.png)   ![good](img/good.png) 1.33    ![regular](img/regular.png)   ![good](img/good.png) 1.33
                  5.33 MiB                      5.33 MiB                      MiB                           5.33 MiB                      MiB

  **2048×2048**   ![bad](img/bad.png) 21.33 MiB ![bad](img/bad.png) 21.33 MiB ![regular](img/regular.png)   ![bad](img/bad.png) 21.33 MiB ![regular](img/regular.png)
                                                                              5.33 MiB                                                    5.33 MiB
...
Note that `RGTC (Red-Green Texture Compression)`{.interpreted-text
...
from the texture to be imported with the RG color format, with only the
red and green channels preserved.
`RGTC (Red-Green Texture Compression)`{.interpreted-text role="abbr"}
compression is able to preserve its detail much better, while using the
same amount of memory as a standard RGBA VRAM-compressed texture. This
only has an effect on textures with the **VRAM Compressed** or **Basis
...
- In 2D, a `class_CanvasItemMaterial`{.interpreted-text role="ref"} will
  need to be created and configured to use the **Premul Alpha** blend
  mode on CanvasItems that use this texture. In
  `custom canvas item shaders <doc_canvas_item_shader>`{.interpreted-text
  role="ref"}, `render_mode blend_premul_alpha;` should be used.
- In 3D, a `class_BaseMaterial3D`{.interpreted-text role="ref"} will
  need to be created and configured to use the **Premul Alpha** blend
  mode on materials that use this texture. In
  `custom spatial shaders <doc_spatial_shader>`{.interpreted-text
  role="ref"}, `render_mode blend_premul_alpha;` should be used.
...
### Detect 3D \> Compress To {#doc_importing_images_detect_3d_compress_to}
