# Import process {#doc_import_process}
...
This means Godot\'s own scene and resource file formats (`.tscn`,
`.scn`, `.tres`, `.res`) don\'t have import options you can select in
the Import dock.
::::
...
also possible. Select all of them together in the FileSystem dock and
the exposed parameters will apply to all of them when reimporting.
