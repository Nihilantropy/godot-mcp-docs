# Animation Track types {#doc_animation_track_types}
...
as follows:
  ...
  **Key**                  **Value**
  ...
  `"args"`                 The arguments to pass to the function as an
                           `Array`
  -----------------------------------------------------------------------
...
also do that with `Ctrl + Shift + Mouse wheel`{.interpreted-text
role="kbd"}. Using `Ctrl + Alt + Mouse wheel`{.interpreted-text
role="kbd"} will zoom in and out on the Y axis
::::

![image](img/bezier_curves.webp)

While a keyframe is selected (not the handle), in the right click panel
of the editor, you can select the handle mode:
