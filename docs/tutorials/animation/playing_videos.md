# Playing videos {#doc_playing_videos}
...
with Ogg Vorbis audio) with optional Ogg Vorbis audio tracks. It\'s
possible for extensions to bring support for additional formats.
...
You may find videos with a `.ogg` or `.ogx` extensions, which are
...
2.  Select the VideoStreamPlayer node in the scene tree dock, go to the
    inspector and load a `.ogv` file in the Stream property.
...
Add an AspectRatioContainer node. Make sure it is not a child of any
...
See `doc_viewports`{.interpreted-text role="ref"} and the [GUI in 3D
...
There are some limitations with the current implementation of video
playback in Godot:

- Streaming a video from a URL is not supported.
- Only mono and stereo audio output is supported. Videos with 4, 5.1 and
  7.1 audio channels are supported but down-mixed to stereo.
...
[FFmpeg](https://ffmpeg.org/) (CLI) is a popular open source tool for
this purpose. FFmpeg has a steep learning curve, but it\'s a powerful
tool.
...
multiplexer. It\'s highly recommended to use one of the latest static
daily builds, or build from their master branch to get the latest fixes.
::::
...
The **GOP (Group of Pictures) size** (`-g:v`) is the max interval
between keyframes. Increasing this value can improve compression with
almost no impact on quality. The default size (`12`) is too low for most
types of content, it\'s therefore recommended using higher GOP values
before reducing video quality. Compression benefits will fade away as
the GOP size increases though. Values between `64` and `512` usually
...
Higher GOP sizes will increase max seek times with a sudden increase
when going beyond powers of two starting at `64`. Max seek times with
GOP size `65` can be almost twice as long as with GOP size `64`,
depending on decoding speed.
::::
...
func [on_color_picker_button_color_changed]{#on_color_picker_button_color_changed}(color):
...
func [on_h_slider_value_changed]{#on_h_slider_value_changed}(value):
...
func [on_h_slider_2_value_changed]{#on_h_slider_2_value_changed}(value):
...
func [on_video_stream_player_finished]{#on_video_stream_player_finished}():
...
> material.
> [videoStreamPlayer.Material.Set]{#videostreamplayer.material.set}(\"shader_parameter/chroma_key_color\",
> color); }
>
> private void OnHSliderValueChanged(double value) { // Update the
> \"pickup_range\" shader parameter of the VideoStreamPlayer\'s
> material.
> [videoStreamPlayer.Material.Set]{#videostreamplayer.material.set}(\"shader_parameter/pickup_range\",
> value); }
>
...
> [videoStreamPlayer.Material.Set]{#videostreamplayer.material.set}(\"shader_parameter/fade_amount\",
> value); }
>
> private void OnVideoStreamPlayerFinished() { // Restart the video
> playback when it\'s finished.
> [videoStreamPlayer.Play]{#videostreamplayer.play}(); }
