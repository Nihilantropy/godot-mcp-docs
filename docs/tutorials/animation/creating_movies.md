# Creating movies {#doc_creating_movies}
...
The path specified in the project settings or metadata can be either
...
method to run a second instance of the exported project that will record
a video file.
::::
...
Make sure to resize the window *before* the splash screen has ended. For
this purpose, it\'s recommended to adjust the **Display \> Window \>
...
[Ready]{#ready}() { if (OS.HasFeature(\"movie\")) { // When recording a
movie, improve SDFGI cell density // without decreasing its maximum
distance. GetViewport().World3D.Environment.SdfgiMinCellSize \*= 0.25f;
