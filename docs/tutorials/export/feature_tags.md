...
**case-sensitive**:
  ...
  **movie**            `Movie Maker mode <doc_creating_movies>`{.interpreted-text
                       role="ref"} is active
  ---------------------------------------------------------------------------------
...
they will *not* change depending on runtime conditions. For example,
...
They are **not used** when running the project from the editor, even if
the export preset marked as **Runnable** for your current platform has
custom feature tags defined.

Custom feature tags are also not used in
`class_EditorExportPlugin`{.interpreted-text role="ref"} scripts.
Instead, feature tags in `class_EditorExportPlugin`{.interpreted-text
role="ref"} will reflect the device the editor is currently running on.
::::
...
<figure>
<img src="img/feature_tags2.webp" alt="img/feature_tags2.webp" />
<figcaption>The desired configuration is selected, which effectively
copies its properties to the panel above (1). The "demo_build" feature
tag is selected (2). The configuration is added to the project settings
(3).</figcaption>
</figure>
...
role="ref"} (which is unrelated to feature tags), remember that feature
tags still apply. Therefore, make sure to *also* override the setting
with the desired feature tag(s) if you want them to override base
...
project settings using the typical approaches
(`ProjectSettings.get_setting<class_ProjectSettings_method_get_setting>`{.interpreted-text
role="ref"} or
`ProjectSettings.get <class_Object_private_method__get>`{.interpreted-text
role="ref"}). Instead, you must use
`ProjectSettings.get_setting_with_override <class_ProjectSettings_method_get_setting>`{.interpreted-text
role="ref"}.
...
Using `ProjectSettings.get_setting("section/subsection/example")` will
return `"Release"` regardless of whether a debug build is currently
running. On the other hand,
`ProjectSettings.get_setting_with_override("section/subsection/example")`
