# A better XR start script {#doc_a_better_xr_start_script}
...
>
> private OpenXRInterface [xrInterface]{#xrinterface};
>
> private bool [xrIsFocused]{#xrisfocused};
...
[ready]{#ready}(): xr_interface = XRServer.find_interface(\"OpenXR\") if
...
> xr_interface.session_begun.connect([on_openxr_session_begun]{#on_openxr_session_begun})
> xr_interface.session_visible.connect([on_openxr_visible_state]{#on_openxr_visible_state})
> xr_interface.session_focussed.connect([on_openxr_focused_state]{#on_openxr_focused_state})
> xr_interface.session_stopping.connect([on_openxr_stopping]{#on_openxr_stopping})
> xr_interface.pose_recentered.connect([on_openxr_pose_recentered]{#on_openxr_pose_recentered})
...
> first time. /// \</summary\> public override void [Ready]{#ready}() {
> [xrInterface]{#xrinterface} =
> (OpenXRInterface)XRServer.FindInterface(\"OpenXR\"); if
> ([xrInterface]{#xrinterface} != null &&
> [xrInterface.IsInitialized]{#xrinterface.isinitialized}()) {
...
> >
> > // Connect the OpenXR events
> > [xrInterface.SessionBegun]{#xrinterface.sessionbegun} +=
> > OnOpenXRSessionBegun;
> > [xrInterface.SessionVisible]{#xrinterface.sessionvisible} +=
> > OnOpenXRVisibleState;
> > [xrInterface.SessionFocussed]{#xrinterface.sessionfocussed} +=
> > OnOpenXRFocusedState;
> > [xrInterface.SessionStopping]{#xrinterface.sessionstopping} +=
> > OnOpenXRStopping;
> > [xrInterface.PoseRecentered]{#xrinterface.poserecentered} +=
...
The main thing we do here is to check our headset\'s refresh rate. We
also check the available refresh rates reported by the XR runtime to
...
\# Handle OpenXR session ready func
[on_openxr_session_begun]{#on_openxr_session_begun}() -\> void: \# Get
the reported refresh rate var current_refresh_rate =
...
> /// \<summary\> /// Handle OpenXR session ready /// \</summary\>
> private void OnOpenXRSessionBegun() { // Get the reported refresh rate
> var currentRefreshRate =
> [xrInterface.DisplayRefreshRate]{#xrinterface.displayrefreshrate};
...
> > [xrInterface.GetAvailableDisplayRefreshRates]{#xrinterface.getavailabledisplayrefreshrates}();
> > if (availableRates.Count == 0) { GD.Print(\"OpenXR: Target does not
...
> > [xrInterface.DisplayRefreshRate]{#xrinterface.displayrefreshrate} =
> > newRate; currentRefreshRate = newRate; }
> >
...
not focused. This is a bit of a weird description in OpenXR but it
basically means that our game has just started and we\'re about to
switch to the focused state next, that the user has opened a system menu
or the user has just took their headset off.

On receiving this signal we\'ll update our focused state, we\'ll change
the process mode of our node to disabled which will pause processing on
this node and its children, and emit our `focus_lost` signal.
...
\# Handle OpenXR visible state func
[on_openxr_visible_state]{#on_openxr_visible_state}() -\> void: \# We
always pass this state at startup, \# but the second time we get this it
means our player took off their headset if xr_is_focussed:
...
> off their headset if ([xrIsFocused]{#xrisfocused}) { GD.Print(\"OpenXR
> lost focus\");
>
> > [xrIsFocused]{#xrisfocused} = false;
> >
...
interacting with your game. Also when the game returns to the focused
mode, suddenly all controller and hand tracking is re-enabled and could
have game breaking consequences if you do not react to this accordingly.
Be sure to test this behavior in your game!
::::

While handling our signal we will update the focuses state, unpause our
...
\# Handle OpenXR focused state func
[on_openxr_focused_state]{#on_openxr_focused_state}() -\> void:
print(\"OpenXR gained focus\") xr_is_focussed = true
...
> focus\"); [xrIsFocused]{#xrisfocused} = true;
>
...
\# Handle OpenXR stopping state func
[on_openxr_stopping]{#on_openxr_stopping}() -\> void: \# Our session is
being stopped. print(\"OpenXR is stopping\")
...
[on_openxr_pose_recentered]{#on_openxr_pose_recentered}() -\> void: \#
