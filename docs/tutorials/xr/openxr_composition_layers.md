# OpenXR composition layers {#doc_openxr_composition_layers}
...
composition layer node as a child node of our
...
If we\'ve just released our button we also prepare an
...
Or if we\'ve just pressed our button we prepare an
`InputEventMouseButton <class_InputEventMouseButton>`{.interpreted-text
...
<figcaption>Use case showing how the user's hand is incorrectly obscured
