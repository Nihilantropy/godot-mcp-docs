# Basic XR Locomotion {#doc_basic_xr_locomotion}
...
`PlaneMesh <class_planemesh>`{.interpreted-text role="ref"} as its mesh.
...
white floor isn\'t going to do it. Create a texture using [Wahooneys
excellent free texture
generator](https://wahooney.itch.io/texture-grid-generator). Once
you\'ve created the texture add it to your project. Then create a new
material for the MeshInstance3D node, add your texture as the albedo,
and enable **Triplaner** under **UV1** in the material properties.
...
also adjust the orientation by using the left hand controller\'s
joystick.
