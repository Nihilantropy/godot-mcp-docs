...
Engines like Godot provide increased ease of use thanks to their
high-level constructs and features. Most of them are accessed and used
via the `Scene System<doc_scene_tree>`{.interpreted-text role="ref"}.
Using nodes and resources simplifies project organization and asset
...
references to be kept around. private Texture2D [texture]{#texture};

> public override void [Ready]{#ready}() { // Create a canvas item,
> child of this node. Rid ciRid = RenderingServer.CanvasItemCreate(); //
> Make this node the parent. RenderingServer.CanvasItemSetParent(ciRid,
> GetCanvasItem()); // Draw a texture on it. // Remember, keep this
> reference. [texture]{#texture} =
> ResourceLoader.Load\<Texture2D\>(\"res://MyTexture.png\"); // Add it,
> centered. RenderingServer.CanvasItemAddTextureRect(ciRid, new
> Rect2(-[texture.GetSize]{#texture.getsize}() / 2,
> [texture.GetSize]{#texture.getsize}()),
> [texture.GetRid]{#texture.getrid}()); // Add the item, rotated 45
> degrees and translated. Transform2D xform =
...
references to be kept around. private Mesh [mesh]{#mesh};

> public override void [Ready]{#ready}() { // Create a visual instance
> (for 3D). Rid instance = RenderingServer.InstanceCreate(); // Set the
> scenario from the world, this ensures it // appears with the same
> objects as the scene. Rid scenario = GetWorld3D().Scenario;
> RenderingServer.InstanceSetScenario(instance, scenario); // Add a mesh
> to it. // Remember, keep the reference. [mesh]{#mesh} =
> ResourceLoader.Load\<Mesh\>(\"res://MyMesh.obj\");
> RenderingServer.InstanceSetBase(instance,
> [mesh.GetRid]{#mesh.getrid}()); // Move the mesh around. Transform3D
> xform = new Transform3D(Basis.Identity, new Vector3(20, 100, 0));
...
func [body_moved]{#body_moved}(state, index):
...
> RenderingServer.CanvasItemSetTransform([canvasItem]{#canvasitem},
> state.Transform); }
>
> public override void [Ready]{#ready}() { // Create the body. var body
