# CPU optimization {#doc_cpu_optimization}
...
`GDScript <doc_gdscript>`{.interpreted-text role="ref"} is designed to
be easy to use and iterate, and is ideal for making many types of games.
However, in this language, ease of use is considered more important than
performance. If you need to make heavy calculations, consider moving
some of your project to one of the other languages.

### C#

`C# <doc_c_sharp>`{.interpreted-text role="ref"} is popular and has
first-class support in Godot. It offers a good compromise between speed
and ease of use. Beware of possible garbage collection pauses and leaks
that can occur during gameplay, though. A common approach to workaround
issues with garbage collection is to use *object pooling*, which is
...
frames to match the physics. Godot has built-in physics interpolation
which you can read about
`here<doc_physics_interpolation>`{.interpreted-text role="ref"}.
