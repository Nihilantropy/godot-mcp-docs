# Shader preprocessor {#doc_shader_preprocessor}
...
`visual shaders <doc_visual_shaders>`{.interpreted-text role="ref"}. If
you need to introduce preprocessor statements to a visual shader, you
can convert it to a text-based shader using the **Convert to Shader**
option in the VisualShader inspector resource dropdown. This conversion
is a one-way operation; text shaders cannot be converted back to visual
shaders.
::::
...
If the replacement code is not defined, the identifier may only be used
...
be continued by an `#else` block, but **must** be ended with the
`#endif` directive.
...
It may be continued by an `#else` block, but must be ended with the
`#endif` directive.
...
because all runtime branches in a shader are still compiled and
...
continued by an `#else` block, but must be ended with the `#endif`
directive.
