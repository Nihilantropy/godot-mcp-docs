...
## Render modes
  ...
  **blend_disabled**          Disable blending, values (including alpha) are
                              written as-is.
  ...
  **skip_vertex_transform**   `VERTEX` needs to be transformed manually in the
                              `vertex()` function.

  **world_vertex_coords**     `VERTEX` is modified in world coordinates
                              instead of local.
  ----------------------------------------------------------------------------
...
- **z**: Animation frame.
  ...
  Built-in                 Description
  ------------------------ ----------------------------------------------------------------------------
  in mat4 **MODEL_MATRIX** Local space to world space transform. World space is the coordinates you
                           normally use in the editor.

  in mat4                  World space to canvas space transform. In canvas space the origin is the
  **CANVAS_MATRIX**        upper-left corner of the screen and coordinates ranging from `(0.0, 0.0)` to
                           viewport size.

  in mat4                  Canvas space to clip space. In clip space coordinates ranging from
  **SCREEN_MATRIX**        `(-1.0, -1.0)` to `(1.0, 1.0).`
  ...
  **INSTANCE_CUSTOM**      
  ...
  in bool                  Always `false`.
  **AT_LIGHT_PASS**        

  in vec2                  Normalized pixel size of default 2D texture. For a Sprite2D with a texture
  **TEXTURE_PIXEL_SIZE**   of size 64x32px, **TEXTURE_PIXEL_SIZE** = `vec2(1/64, 1/32)`
  ...
  inout vec4 **COLOR**     Color from vertex primitive multiplied by CanvasItem\'s
                           `modulate<class_CanvasItem_property_modulate>`{.interpreted-text role="ref"}
...
```
  ...
  in vec4 **FRAGCOORD**            Coordinate of pixel center. In screen
                                   space. `xy` specifies position in
                                   viewport. Upper-left of the viewport is
                                   the origin, `(0.0, 0.0)`.

  in vec2 **SCREEN_PIXEL_SIZE**    Size of individual pixels. Equal to
  ...
  in vec2 **TEXTURE_PIXEL_SIZE**   Normalized pixel size of default 2D
                                   texture. For a Sprite2D with a texture of
                                   size 64x32px, `TEXTURE_PIXEL_SIZE` =
                                   `vec2(1/64, 1/32)`

  in bool **AT_LIGHT_PASS**        Always `false`.

  sampler2D                        Specular shininess texture of this
  **SPECULAR_SHININESS_TEXTURE**   object.

  in vec4 **SPECULAR_SHININESS**   Specular shininess color, as sampled from
                                   the texture.
  ...
  sampler2D **SCREEN_TEXTURE**     Removed in Godot 4. Use a `sampler2D`
                                   with `hint_screen_texture` instead.

  inout vec3 **NORMAL**            Normal read from `NORMAL_TEXTURE`.
                                   Writable.

  sampler2D **NORMAL_TEXTURE**     Default 2D normal texture.

  out vec3 **NORMAL_MAP**          Configures normal maps meant for 3D for
                                   use in 2D. If used, overrides `NORMAL`.

  out float **NORMAL_MAP_DEPTH**   Normal map depth for scaling.

  inout vec2 **VERTEX**            Pixel position in screen space.

  inout vec2 **SHADOW_VERTEX**     Same as `VERTEX` but can be written to
                                   alter shadows.

  inout vec3 **LIGHT_VERTEX**      Same as `VERTEX` but can be written to
                                   alter lighting. Z component represents
                                   height.

  inout vec4 **COLOR**             `COLOR` from the `vertex()` function
                                   multiplied by the `TEXTURE` color. Also
                                   output color value.
  --------------------------------------------------------------------------
...
```
  ...
  in vec4 **FRAGCOORD**      Coordinate of pixel center. In screen space. `xy` specifies position
                             in viewport. Upper-left of the viewport is the origin, `(0.0, 0.0)`.
  ...
  in vec2                    Normalized pixel size of `TEXTURE`. For a Sprite2D with a `TEXTURE` of
  **TEXTURE_PIXEL_SIZE**     size `64x32` pixels, **TEXTURE_PIXEL_SIZE** = `vec2(1/64, 1/32)`

  in vec2 **SCREEN_UV**      Screen UV coordinate for current pixel.

  in vec2 **POINT_COORD**    UV for Point Sprite.

  in vec4 **LIGHT_COLOR**    `Color<class_Light2D_property_color>`{.interpreted-text role="ref"} of
                             the `class_Light2D`{.interpreted-text role="ref"}. If the light is a
                             `class_PointLight2D`{.interpreted-text role="ref"}, multiplied by the
                             light\'s
                             `texture<class_PointLight2D_property_texture>`{.interpreted-text
                             role="ref"}.

  in float **LIGHT_ENERGY**  `Energy multiplier<class_Light2D_property_energy>`{.interpreted-text
                             role="ref"} of the `class_Light2D`{.interpreted-text role="ref"}.

  in vec3 **LIGHT_POSITION** Position of the `class_Light2D`{.interpreted-text role="ref"} in
                             screen space. If using a `class_DirectionalLight2D`{.interpreted-text
                             role="ref"} this is always `(0.0, 0.0, 0.0)`.

  in vec3                    Direction of the `class_Light2D`{.interpreted-text role="ref"} in
  **LIGHT_DIRECTION**        screen space.

  in bool                    `true` if this pass is a `class_DirectionalLight2D`{.interpreted-text
  **LIGHT_IS_DIRECTIONAL**   role="ref"}.

  in vec3 **LIGHT_VERTEX**   Pixel position, in screen space as modified in the `fragment()`
                             function.

  inout vec4 **LIGHT**       Output color for this `class_Light2D`{.interpreted-text role="ref"}.

  in vec4                    Specular shininess, as set in the object\'s texture.
  **SPECULAR_SHININESS**     

  out vec4                   Multiply shadows cast at this point by this color.
  **SHADOW_MODULATE**        
  -------------------------------------------------------------------------------------------------
...
The signed distance field is generated from
...
role="ref"} documentation for more information.
  ...
  vec2 **texture_sdf_normal** (vec2     Calculates a normal from the SDF
  sdf_pos)                              texture.

  vec2 **sdf_to_screen_uv** (vec2       Converts an SDF to screen UV.
  sdf_pos)                              
