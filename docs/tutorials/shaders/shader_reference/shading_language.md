...
## Data types {#doc_shading_language_data_types}
...
Most GLSL ES 3.0 datatypes are supported:
  ...
  **sampler2D**            Sampler type for binding 2D textures, which are read
                           as float.

  **isampler2D**           Sampler type for binding 2D textures, which are read
                           as signed integer.

  **usampler2D**           Sampler type for binding 2D textures, which are read
                           as unsigned integer.
  ...
  **sampler3D**            Sampler type for binding 3D textures, which are read
                           as float.

  **isampler3D**           Sampler type for binding 3D textures, which are read
                           as signed integer.

  **usampler3D**           Sampler type for binding 3D textures, which are read
                           as unsigned integer.

  **samplerCube**          Sampler type for binding Cubemaps, which are read as
                           float.

  **samplerCubeArray**     Sampler type for binding Cubemap arrays, which are
                           read as float. Only supported in Forward+ and Mobile,
                           not Compatibility.

  **samplerExternalOES**   External sampler type. Only supported in
                           Compatibility/Android platform.
  -------------------------------------------------------------------------------
...
value was already present at that memory location, and unpredictable
visual glitches will appear. However, uniforms and varyings are
...
C++, using `//` for single-line comments and `/* */` for multi-line
comments:
...
a constant expression or a number, the editor will check its bounds to
prevent this crash.
::::
...
Most operators that accept vectors or matrices (multiplication,
division, etc) operate component-wise, meaning the function is applied
to the first value of each vector and then on the second value of each
vector, etc. Some examples:
...
## Uniforms {#doc_shading_language_uniforms}

Passing values to shaders is possible with *uniforms*, which are defined
in the global scope of the shader, outside of functions. When a shader
is later assigned to a material, the uniforms will appear as editable
parameters in the material\'s inspector. Uniforms can\'t be written from
within the shader. Any
`data type <doc_shading_language_data_types>`{.interpreted-text
role="ref"} except for `void` can be a uniform.
...
You can set uniforms in the editor in the material\'s inspector.
Alternately, you can set them
`from code <doc_shading_language_setting_uniforms_from_code>`{.interpreted-text
role="ref"}.
...
Full list of uniform hints below:
  ...
  **int**         hint_enum(\"String1\", \"String2\")                                       Displays int input as a dropdown
                                                                                            widget in the editor.

  **int, float**  hint_range(min, max\[, step\])                                            Restricted to values in a range
                                                                                            (with min/max/step).
  ...
  **sampler2D**   hint_default_white                                                        As value or albedo color, default to
                                                                                            opaque white.

  **sampler2D**   hint_default_black                                                        As value or albedo color, default to
                                                                                            opaque black.

  **sampler2D**   hint_default_transparent                                                  As value or albedo color, default to
                                                                                            transparent black.

  **sampler2D**   hint_anisotropy                                                           As flowmap, default to right.

  **sampler2D**   hint_roughness\[[r]{#r}, [g]{#g}, [b]{#b}, [a]{#a}, [normal]{#normal},    Used for roughness limiter on import
                  [gray]{#gray}\]                                                           (attempts reducing specular
                                                                                            aliasing). `_normal` is a normal map
                                                                                            that guides the roughness limiter,
                                                                                            with roughness increasing in areas
                                                                                            that have high-frequency detail.

  **sampler2D**   filter\[[nearest]{#nearest},                                              Enabled specified texture filtering.
                  [linear]{#linear}\]\[[mipmap]{#mipmap}\]\[[anisotropic]{#anisotropic}\]   

  **sampler2D**   repeat\[[enable]{#enable}, [disable]{#disable}\]                          Enabled texture repeating.

  **sampler2D**   hint_screen_texture                                                       Texture is the screen texture.

  **sampler2D**   hint_depth_texture                                                        Texture is the depth texture.

  **sampler2D**   hint_normal_roughness_texture                                             Texture is the normal roughness
                                                                                            texture (only supported in
                                                                                            Forward+).
  ------------------------------------------------------------------------------------------------------------------------------
...
The value will be stored as an integer, corresponding to the index of
the selected option (i.e. `0`, `1`, or `2`) or the value assigned by
colon syntax (i.e. `30`, `60`, or `200`). When setting the value with
...
shader. While regular uniforms are defined outside of shader functions
and are therefore the global scope of the shader, global uniforms are
global to all shaders in the entire project (but within each shader, are
...
While you *can* query the value of a global uniform at runtime in a
script using
...
`autoload <doc_singletons_autoload>`{.interpreted-text role="ref"} where
you store the values you need to query at the same time you\'re setting
...
### Setting uniforms from code {#doc_shading_language_setting_uniforms_from_code}

You can set uniforms from GDScript using the
`set_shader_parameter() <class_ShaderMaterial_method_set_shader_parameter>`{.interpreted-text
role="ref"} method:
...
| **bool**     | **bool**      |                                     |
+--------------+---------------+-------------------------------------+
| **bvec2**    | **int**       | Bitwise packed int where bit 0      |
|              |               | (LSB) corresponds to x.             |
|              |               |                                     |
|              |               | For example, a bvec2 of (bx, by)    |
|              |               | could be created in the following   |
|              |               | way:                                |
|              |               |                                     |
|              |               | ``` gdscript                        |
|              |               | bvec2_inpu                          |
|              |               | t: int = (int(bx)) | (int(by) << 1) |
|              |               | ```                                 |
+--------------+---------------+-------------------------------------+
| **bvec3**    | **int**       | Bitwise packed int where bit 0      |
|              |               | (LSB) corresponds to x.             |
+--------------+---------------+-------------------------------------+
| **bvec4**    | **int**       | Bitwise packed int where bit 0      |
|              |               | (LSB) corresponds to x.             |
+--------------+---------------+-------------------------------------+
| **int**      | **int**       |                                     |
+--------------+---------------+-------------------------------------+
...
| **vec2**     | **Vector2**   |                                     |
+--------------+---------------+-------------------------------------+
| **vec3**     | **Vector3**,  | When Color is used, it will be      |
|              | **Color**     | interpreted as (r, g, b).           |
+--------------+---------------+-------------------------------------+
| **vec4**     | **Vector4**,  | When Color is used, it will be      |
|              | **Color**,    | interpreted as (r, g, b, a).        |
|              | **Rect2**,    |                                     |
|              | **Plane**,    | When Rect2 is used, it will be      |
|              | *             | interpreted as (position.x,         |
|              | *Quaternion** | position.y, size.x, size.y).        |
|              |               |                                     |
|              |               | When Plane is used it will be       |
|              |               | interpreted as (normal.x, normal.y, |
|              |               | normal.z, d).                       |
+--------------+---------------+-------------------------------------+
| **mat2**     | **            |                                     |
|              | Transform2D** |                                     |
...
| **mat3**     | **Basis**     |                                     |
+--------------+---------------+-------------------------------------+
| **mat4**     | **            | When a Transform3D is used, the w   |
|              | Projection**, | Vector is set to the identity.      |
|              | **            |                                     |
|              | Transform3D** |                                     |
+--------------+---------------+-------------------------------------+
| *            | **Texture2D** |                                     |
...
+--------------+---------------+-------------------------------------+
| **s          | **Cubemap**   | See                                 |
| amplerCube** |               | `doc_importing_images_chan          |
|              |               | ging_import_type`{.interpreted-text |
|              |               | role="ref"} for instructions on     |
|              |               | importing cubemaps for use in       |
|              |               | Godot.                              |
+--------------+---------------+-------------------------------------+
| **sample     | **C           | Only supported in Forward+ and      |
| rCubeArray** | ubemapArray** | Mobile, not Compatibility.          |
+--------------+---------------+-------------------------------------+
| **samplerE   | **Exte        | Only supported in                   |
| xternalOES** | rnalTexture** | Compatibility/Android platform.     |
+--------------+---------------+-------------------------------------+
...
Be careful when setting shader uniforms from GDScript, since no error
...
int/float (64 bit) into a Godot shader language int/float (32 bit). This
may lead to unintended consequences in cases where high precision is
required.
::::
...
smaller than a `vec4`, such as `vec2` or `vec3`, are padded to the size
of a `vec4`. Scalar uniforms such as `int` or `float` are not padded,
and `bool` is padded to the size of an `int`.
...
(`vertex`, `fragment`, `light`, `start`, `process`, `sky`, or `fog`).
For a list of the built-in variables that are available, please see the
corresponding pages:
...
ES 3.0. See the
`Built-in functions <doc_shader_functions>`{.interpreted-text
