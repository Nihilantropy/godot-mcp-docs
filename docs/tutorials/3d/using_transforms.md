# Using 3D transforms {#doc_using_transforms}
...
The axis should be defined in the local coordinate system of the object.
For example, to rotate around the object\'s local X, Y, or Z axes, use
...
// accumulators private float [rotationX]{#rotationx} = 0f; private
float [rotationY]{#rotationy} = 0f;

public override void [Input]{#input}(InputEvent \@event) { if (@event is
InputEventMouseMotion mouseMotion) { // modify accumulated mouse
rotation [rotationX]{#rotationx} += mouseMotion.Relative.X \*
LookAroundSpeed; [rotationY]{#rotationy} += mouseMotion.Relative.Y \*
LookAroundSpeed;

> // reset rotation Transform3D transform = Transform; transform.Basis =
> Basis.Identity; Transform = transform;
>
> RotateObjectLocal(Vector3.Up, [rotationX]{#rotationx}); // first
> rotate about Y RotateObjectLocal(Vector3.Right,
> [rotationY]{#rotationy}); // then rotate about X }
...
// Convert basis to quaternion, keep in mind scale is lost var a = new
Quaternion(transform.Basis); var b = new Quaternion(transform2.Basis);
// Interpolate using spherical-linear interpolation (SLERP). var c =
