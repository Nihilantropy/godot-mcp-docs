# 3D rendering limitations {#doc_3d_rendering_limitations}
...
renders internally in HDR. However, the rendering output will be
...
using the Mobile rendering method compared to Forward+.
...
- If using the Forward+ or Forward Mobile rendering methods, enable
  `Use Debanding<class_ProjectSettings_property_rendering/anti_aliasing/quality/use_debanding>`{.interpreted-text
  role="ref"} in **Project Settings \> Rendering \> Anti Aliasing**.
  This applies a fullscreen debanding shader as a post-processing effect
  and is very cheap.
