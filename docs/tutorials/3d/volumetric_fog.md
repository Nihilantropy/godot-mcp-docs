# Volumetric fog and fog volumes {#doc_volumetric_fog}
...
Volumetric fog is only supported in the Forward+ renderer, not the
...
Volumetric Fog \> Volume Depth** project setting (at a performance cost)
or by decreasing **Length** in the Environment volumetric fog properties
(at no performance cost, but at the cost of lower fog range).
Alternatively, the FogVolume can be made thicker and use a lower density
in the **Material**.
::::
...
- Quads work with any rendering method, including Mobile and
  Compatibility.
