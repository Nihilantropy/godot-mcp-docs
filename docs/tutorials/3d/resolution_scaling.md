# Resolution scaling {#doc_resolution_scaling}
...
- **Bilinear:** Standard bilinear filtering (default). This is used as a
  fallback when the current renderer doesn\'t support FSR 1.0 or FSR
  2.2. *Available in all renderers.*
...
- **FSR 2.2:** AMD FidelityFX Super Resolution 2.2 (since Godot 4.2).
  ...
  factor. *Only available when using the Forward+ renderer.*
...
**1920×1080 (Full HD)**
  ...
  -----------------------------------------------------------------------
  Resolution scale       3D rendering          Megapixels rendered per
  factor                 resolution            frame
  ---------------------- --------------------- --------------------------
...
advanced project setting. It can also be changed at runtime on
