# Using the ArrayMesh {#doc_arraymesh}
...
List\<Vector3\> normals = \[\]; List\<int\> indices = \[\];
::::
...
void [Ready]{#ready}() { Godot.Collections.Array surfaceArray = \[\];
surfaceArray.Resize((int)Mesh.ArrayType.Max);

> // C# arrays cannot be resized or expanded, so use Lists to create
> geometry. List\<Vector3\> verts = \[\]; List\<Vector2\> uvs = \[\];
> List\<Vector3\> normals = \[\]; List\<int\> indices = \[\];
>
> /\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*
> \* Insert code here to generate mesh. \*
...
[rings]{#rings} = 50; private int [radialSegments]{#radialsegments} =
50; private float [radius]{#radius} = 1;

> public override void [Ready]{#ready}() { // Insert setting up the
> surface array and lists here.
>
> > // Vertex indices. var thisRow = 0; var prevRow = 0; var point = 0;
> >
> > // Loop over rings. for (var i = 0; i \< [rings]{#rings} + 1; i++) {
> > var v = ((float)i) / [rings]{#rings}; var w = Mathf.Sin(Mathf.Pi \*
> > v); var y = Mathf.Cos(Mathf.Pi \* v);
> >
> > > // Loop over segments in ring. for (var j = 0; j \<
> > > [radialSegments]{#radialsegments} + 1; j++) { var u = ((float)j) /
> > > [radialSegments]{#radialsegments}; var x = Mathf.Sin(u \* Mathf.Pi
> > > \* 2); var z = Mathf.Cos(u \* Mathf.Pi \* 2); var vert = new
> > > Vector3(x \* [radius]{#radius} \* w, y \* [radius]{#radius}, z \*
> > > [radius]{#radius} \* w); verts.Add(vert);
> > > normals.Add(vert.Normalized()); uvs.Add(new Vector2(u, v)); point
> > > += 1;
> > >
