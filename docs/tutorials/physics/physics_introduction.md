...
Names can be assigned in **Project Settings \> Layer Names \> 2D
Physics**.
...
`set_collision_mask_value(layer_number, value)` on any given
`CollisionObject2D <class_CollisionObject2D>`{.interpreted-text
role="ref"} as follows:
...
Export annotations can be used to export bitmasks in the editor with a
user-friendly GUI:

    @export_flags_2d_physics var layers_2d_physics

Additional export annotations are available for render and navigation
layers, in both 2D and 3D. See
`doc_gdscript_exports_exporting_bit_flags`{.interpreted-text
role="ref"}.
...
in **Project Settings \> Physics**, or by entering an
...
func [integrate_forces]{#integrate_forces}(state):
...
public partial class Spaceship : RigidBody2D { private Vector2
[thrust]{#thrust} = new Vector2(0, -250); private float
[torque]{#torque} = 20000;

> public override void
> [IntegrateForces]{#integrateforces}(PhysicsDirectBodyState2D state) {
> if (Input.IsActionPressed(\"ui_up\")) {
> state.ApplyForce([thrust.Rotated]{#thrust.rotated}(Rotation)); } else
...
> > rotationDir -= 1; } state.ApplyTorque(rotationDir \*
> > [torque]{#torque});
>
> }
...
#### `move_and_collide <class_PhysicsBody2D_method_move_and_collide>`{.interpreted-text role="ref"}
...
public partial class Body : PhysicsBody2D { private Vector2
[velocity]{#velocity} = new Vector2(250, 250);

> public override void [PhysicsProcess]{#physicsprocess}(double delta) {
> var collisionInfo = MoveAndCollide([velocity]{#velocity} \*
> (float)delta); if (collisionInfo != null) { var collisionPoint =
...
> public override void [PhysicsProcess]{#physicsprocess}(double delta) {
> var collisionInfo = MoveAndCollide([velocity]{#velocity} \*
> (float)delta); if (collisionInfo != null) { [velocity]{#velocity} =
> [velocity.Bounce]{#velocity.bounce}(collisionInfo.GetNormal()); } }
...
public partial class Body : CharacterBody2D { private float
[runSpeed]{#runspeed} = 350; private float [jumpSpeed]{#jumpspeed} =
-1000; private float [gravity]{#gravity} = 2500;
...
> >
> > if (IsOnFloor() && jump) { velocity.Y = [jumpSpeed]{#jumpspeed}; }
> > if (right) { velocity.X += [runSpeed]{#runspeed}; } if (left) {
> > velocity.X -= [runSpeed]{#runspeed}; }
> >
> > Velocity = velocity;
>
> }
>
> public override void [PhysicsProcess]{#physicsprocess}(double delta) {
> var velocity = Velocity; velocity.Y += [gravity]{#gravity} \*
> (float)delta; Velocity = velocity; GetInput(); MoveAndSlide(); }
