# Collision shapes (2D) {#doc_collision_shapes_2d}
...
role="ref"} as *direct* child nodes. Indirect child nodes (i.e. children
of child nodes) will be ignored and won\'t be used as collision shapes.
Also, note that you must add a `class_Shape2D`{.interpreted-text
role="ref"} *resource* to collision shape nodes in the Inspector dock.
...
polygon. This can result in better performance and game feel, since the
player won\'t be blocked by small, decorative details.
