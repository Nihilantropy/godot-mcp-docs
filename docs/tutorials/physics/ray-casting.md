...
var spaceRid = GetWorld2D().Space; var spaceState =
...
public override void [PhysicsProcess]{#physicsprocess}(double delta) {
var spaceState = GetWorld2D().DirectSpaceState; // use global
coordinates, not local to node var query =
PhysicsRayQueryParameters2D.Create(Vector2.Zero, new Vector2(50, 100));
...
if (result.Count \> 0) { GD.Print(\"Hit at point: \",
result\[\"position\"\]); }
::::
...
public override void [PhysicsProcess]{#physicsprocess}(double delta) {
var spaceState = GetWorld3D().DirectSpaceState; var cam =
...
override void [PhysicsProcess]{#physicsprocess}(double delta) { var
spaceState = GetWorld2D().DirectSpaceState; var query =
PhysicsRayQueryParameters2D.Create(globalPosition, playerPosition);
query.Exclude = \[GetRid()\]; var result =
...
CollisionMask, \[GetRid()\]); var result =
spaceState.IntersectRay(query); } }
::::
