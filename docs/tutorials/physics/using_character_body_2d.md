# Using CharacterBody2D/3D {#doc_using_character_body_2d}
...
will not move a [CharacterBody2D]{.title-ref}.
::::
...
This method takes one required parameter: a
...
- `floor_stop_on_slope` - *default value:* `true`
  ...
  > This parameter prevents a body from sliding down slopes when
  > standing still.
...
[get_slide_collision_count()]{.title-ref} only counts times the body has
collided and changed direction.
::::
...
> Velocity = inputDir \* [speed]{#speed}; }
>
> public override void [PhysicsProcess]{#physicsprocess}(double delta) {
> GetInput(); MoveAndCollide(Velocity \* (float)delta); }
...
PackedScene [bullet]{#bullet} =
GD.Load\<PackedScene\>(\"res://Bullet.tscn\"); private int
[speed]{#speed} = 200;

> public void GetInput() { // Add these actions in Project Settings -\>
> Input Map. float inputDir = Input.GetAxis(\"backward\", \"forward\");
> Velocity = Transform.X \* inputDir \* [speed]{#speed}; if
...
>
> public override void [PhysicsProcess]{#physicsprocess}(double delta) {
> GetInput(); var dir = GetGlobalMousePosition() - GlobalPosition; //
> Don\'t move if too close to the mouse pointer. if (dir.Length() \> 5)
> { Rotation = dir.Angle(); MoveAndSlide(); } }
...
func start([position]{#position}, [direction]{#direction}):

:   rotation = [direction]{#direction} position = [position]{#position}
...
func [on_VisibilityNotifier2D_screen_exited]{#on_visibilitynotifier2d_screen_exited}():
...
public partial class Bullet : CharacterBody2D { public int
[speed]{#speed} = 750;
...
>
> public override void [PhysicsProcess]{#physicsprocess}(double delta) {
> var collision = MoveAndCollide(Velocity \* (float)delta); if
> (collision != null) { Velocity =
> Velocity.Bounce(collision.GetNormal()); if
...
[speed]{#speed} = 100.0f; private float [jumpSpeed]{#jumpspeed} =
-400.0f;
...
> > IsOnFloor()) { velocity.Y = [jumpSpeed]{#jumpspeed}; }
> >
