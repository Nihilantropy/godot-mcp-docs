...
## [process]{#process} vs. [physics_process]{#physics_process} vs. \*\_input
...
every frame (or even every fixed frame). func [ready]{#ready}(): var
timer = Timer.new() timer.autostart = true timer.wait_time = 0.5
...
frame). public override void [Ready]{#ready}() { var timer = new
Timer(); timer.Autostart = true; timer.WaitTime = 0.5; AddChild(timer);
...
:   // Allows for recurring operations that don\'t trigger script logic
    // every frame (or even every fixed frame). virtual void
    [ready]{#ready}() override { Timer \*timer = memnew(Timer);
    timer-\>set_autostart(true); timer-\>set_wait_time(0.5);
    add_child(timer); timer-\>connect(\"timeout\", callable_mp(this,
    &MyNode::run)); }
...
[process]{#process}(delta): if
Input.is_action_just_pressed(\"ui_select\"): print(delta)

\# Called during every input event. func
[unhandled_input]{#unhandled_input}(event): match event.get_class():
...
> void [Process]{#process}(double delta) { if
> (Input.IsActionJustPressed(\"ui_select\")) { GD.Print(delta); } }
>
> // Called during every input event. Equally true for
> [input]{#input}(). public void
> [UnhandledInput]{#unhandledinput}(InputEvent \@event) { switch
> (@event) { case InputEventKey: if
...
:   // Called every frame, even when the engine detects no input.
    virtual void [process]{#process}(double p_delta) override { if
    (Input::get_singleton-\>is_action_just_pressed(\"ui_select\")) {
    UtilityFunctions::print(p_delta); } }

    // Called during every input event. Equally true for
    [input]{#input}(). virtual void
    [unhandled_input]{#unhandled_input}(const Ref\<InputEvent\>
    &p_event) override { Ref\<InputEventKey\> key_event = event; if
    (key_event.is_valid() &&
...
## [init]{#init} vs. initialization vs. export
...
2.  `_init()` **assignment:** the property\'s value is replaced by any
...
:   \# Triggers the setter, changing test\'s value from \"one\" to
    \"two!\". test = \"two\"
...
public partial class MyNode : Node { private string [test]{#test} =
\"one\";

> \[Export\] public string Test { get { return [test]{#test}; } set {
> [test]{#test} = \$\"{value}!\"; } }
>
> public MyNode() { // Triggers the setter, changing [test]{#test}\'s
> value from \"one\" to \"two!\". Test = \"two\"; }
>
> // If someone sets Test to \"three\" in the Inspector, it would
> trigger // the setter, changing [test]{#test}\'s value from \"two!\"
> to \"three!\".
...
:   
    ...
    static void [bind_methods]{#bind_methods}() {

    :   ClassDB::bind_method(D_METHOD(\"get_test\"), &MyNode::get_test);
        ClassDB::bind_method(D_METHOD(\"set_test\", \"test\"),
        &MyNode::set_test); ADD_PROPERTY(PropertyInfo(Variant::STRING,
        \"test\"), \"set_test\", \"get_test\");

    }

public:

:   String get_test() { return test; } void set_test(String p_test) {
    return test = p_test; }

    MyNode() {

    :   // Triggers the setter, changing [test]{#test}\'s value from
        \"one\" to \"two!\". set_test(\"two\");
...
## [ready]{#ready} vs. [enter_tree]{#enter_tree} vs. NOTIFICATION_PARENTED
...
:   
    ...
    :   
        ...
        :   parent_cache = get_parent() if connection_check():
            parent_cache.interacted_with.connect([on_parent_interacted_with]{#on_parent_interacted_with})
        ...
        :   
            ...
            :   parent_cache.interacted_with.disconnect([on_parent_interacted_with]{#on_parent_interacted_with})

func [on_parent_interacted_with]{#on_parent_interacted_with}():
...
public partial class MyNode : Node { private Node
[parentCache]{#parentcache};

> public bool ConnectionCheck() { return
> [parentCache.HasUserSignal]{#parentcache.hasusersignal}(\"InteractedWith\");
> }
>
> public override void [Notification]{#notification}(int what) { switch
> ((long)what) { case NotificationParented: [parentCache]{#parentcache}
> = GetParent(); if (ConnectionCheck()) {
> [parentCache.Connect]{#parentcache.connect}(\"InteractedWith\",
> Callable.From(OnParentInteractedWith)); } break; case
> NotificationUnparented: if (ConnectionCheck()) {
> [parentCache.Disconnect]{#parentcache.disconnect}(\"InteractedWith\",
...
:   GDCLASS(MyNode, Node)
    ...
    :   UtilityFunctions::print(\"I\'m reacting to my parent\'s
        interaction!\");
...
:   
    ...
    void [notification]{#notification}(int p_what) {
    ...
    :   
        ...
        :   
            ...
            :   parent_cache = get_parent(); if (connection_check()) {
                parent_cache-\>connect(\"interacted_with\",
                callable_mp(this, &MyNode::on_parent_interacted_with));
                } break;
            ...
            :   
                ...
                :   parent_cache-\>disconnect(\"interacted_with\",
                    callable_mp(this,
                    &MyNode::on_parent_interacted_with));
