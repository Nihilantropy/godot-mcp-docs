# When to use scenes versus scripts {#doc_scenes_versus_scripts}
...
declaring a composition of nodes, and the script adding behavior with
imperative code.
...
[node]{#node}; private Node [myNode]{#mynode}; private Node
[myScene]{#myscene}; private Node [myInheritedScene]{#myinheritedscene};

> public Game() { [node]{#node} = new Node(); [myNode]{#mynode} =
> MyNode.New().As\<Node\>(); // Different than calling new() or
> MyNode.New(). Instantiated from a PackedScene. [myScene]{#myscene} =
> MyScene.Instantiate(); // Create scene inheriting from MyScene.
> [myInheritedScene]{#myinheritedscene} =
...
- If one would like to give a name to a scene, then they can still sort
  ...
  \# main.gd extends Node func [ready]{#ready}():
  ...
  [Ready]{#ready}() { AddChild(Game.MyScene.Instantiate()); } }
  ::::
