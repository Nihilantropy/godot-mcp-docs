# Logic preferences {#doc_logic_preferences}
...
of a node before adding it to the scene tree. There are some exceptions
where values *can\'t* be set before being added to the scene tree, like
...
>
> public override void [Ready]{#ready}() { // Can assign the value
> during initialization. ABuilding =
...
:   const Ref\<PackedScene\> building =
    ResourceLoader::get_singleton()-\>load(\"res://building.tscn\");
    Ref\<PackedScene\> a_building;

    virtual void [ready]{#ready}() override {

    :   // Can assign the value during initialization. a_building =
        ResourceLoader::get_singleton()-\>load(\"res://office.tscn\");
