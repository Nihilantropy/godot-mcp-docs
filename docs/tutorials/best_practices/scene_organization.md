...
1.  Connect to a signal. Extremely safe, but should be used only to
    ...
    // Parent Node \*node = get_node\<Node\>(\"Child\"); if (node !=
    nullptr) { // Note that get_node may return a nullptr, which would
    make calling the connect method crash the engine if \"Child\" does
    not exist! // So unless you are 1000% sure get_node will never
    return a nullptr, it\'s a good idea to always do a nullptr check.
    node-\>connect(\"signal_name\", callable_mp(this,
    &ObjectWithMethod::method_on_the_object)); }
...
2.  Call a method. Used to start behavior.
    ...
    // Parent Node \*node = get_node\<Node\>(\"Child\"); if (node !=
    nullptr) { node-\>set(\"method_name\", \"do\"); }

    // Child call(method_name); // Call parent-defined method (which
    child must own).
    :::::
...
3.  Initialize a `Callable <class_Callable>`{.interpreted-text
    ...
    // Parent Node \*node = get_node\<Node\>(\"Child\"); if (node !=
    nullptr) { node-\>set(\"func_property\",
    Callable(&ObjectWithMethod::method_on_the_object)); }
...
5.  Initialize a NodePath.
    ...
    // Parent Node \*node = get_node\<Node\>(\"Child\"); if (node !=
    nullptr) { node-\>set(\"target_path\", NodePath(\"..\")); }

    // Child get_node\<Node\>(target_path); // Use parent-defined
    NodePath.
    :::::
...
\# Right func [init]{#init}(): var receiver = Receiver.new()
add_child(receiver)
...
// Parent get_node\<Left\>(\"Left\")-\>target =
get_node\<Node\>(\"Right/Receiver\");
...
:   GDCLASS(Right, Node)
    ...
    :   Node \*receiver = nullptr;
        ...
        :   receiver = memnew(Node); add_child(receiver);
...
code that relies on external documentation to use it safely is
...
for nodes such as the `Area2D <class_Area2D>`{.interpreted-text
role="ref"} node when it has no child
...
of every players\' \"PlayerController\" logic - they only need their
own. Keeping them in a separate branch from the \"world\" can help
simplify the management of game connections and the like.
::::
