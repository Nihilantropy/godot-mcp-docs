# GDScript format strings {#doc_gdscript_printf}

Godot offers multiple ways to dynamically change the contents of
strings:
...
placeholder character sequences such as `%s`. These placeholders can
then be replaced by parameters handed to the format string.
...
implicit String conversion or
`str() <class_@GlobalScope_method_str>`{.interpreted-text role="ref"}
would convert it. Strings remain unchanged, booleans turn into either
`"True"` or `"False"`, an `int` or `float` becomes a decimal, and other
types usually return their data in a human-readable string.

There are other [format specifiers](#format-specifiers).
...
parameters.
  ...
  `c`     A single **Unicode character**. Accepts a Unicode code point
          (integer) or a single-character string. Supports values beyond
          255.

  `d`     A **decimal integer**. Expects an integer or a real number
          (will be floored).

  `o`     An **octal integer**. Expects an integer or a real number (will
          be floored).

  `x`     A **hexadecimal integer** with **lower-case** letters. Expects
          an integer or a real number (will be floored).

  `X`     A **hexadecimal integer** with **upper-case** letters. Expects
          an integer or a real number (will be floored).
  ...
  `v`     A **vector**. Expects any float or int-based vector object (
          ...
          `Vector4i`). Will display the vector coordinates in
          parentheses, formatting each coordinate as if it was an `%f`,
          and using the same modifiers.
  ------- ---------------------------------------------------------------
...
certain conditions.
  ...
  Integer   Set **padding**. Padded with spaces or with zeroes if integer
            starts with `0` in an integer or real number placeholder. The
            leading `0` is ignored if `-` is present. When used after
            `.`, see `.`.
  ...
  `*`       **Dynamic padding**, expects additional integer parameter to
...
used, rounding to integer values. The integer to use for padding must
...
`String.format() <class_String_method_format>`{.interpreted-text
role="ref"} method. It replaces all occurrences of a key in the string
with the corresponding value. The method can handle arrays or
...
When using string concatenation, values that are not strings must be
converted using the `str()` function. There is no way to specify the
string format of converted values.
...
Because of these limitations, format strings or the `format()` method
are often a better choice. In many cases, string concatenation is also
less readable.
