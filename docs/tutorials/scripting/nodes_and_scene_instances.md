# Nodes and scene instances {#doc_nodes_and_scene_instances}

This guide explains how to get nodes, create nodes, add them as a child,
and instantiate scenes from code.

::: seealso
Check the `doc_instancing`{.interpreted-text role="ref"} tutorial to
learn about Godot\'s approach to scene instancing.
:::
...
private Sprite2D [sprite2D]{#sprite2d}; private Camera2D
[camera2D]{#camera2d};
...
private Sprite2D [sprite2D]{#sprite2d};

public override void [Ready]{#ready}() { base.\_Ready();

> [sprite2D]{#sprite2d} = new Sprite2D(); // Create a new Sprite2D.
> AddChild([sprite2D]{#sprite2d}); // Add it as a child of this node.
...
[sprite2D.QueueFree]{#sprite2d.queuefree}();
::::
