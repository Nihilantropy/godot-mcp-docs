...
expression doesn\'t capture a variable, the generated delegate\'s
`Target` is the instance that created the delegate. When a variable is
captured, the `Target` instead points at a generated type that stores
the captured variable. This is what breaks the association. If you want
to see if a delegate will be automatically cleaned up, try checking its
`Target`.
