# Pausing games and process mode {#doc_pausing_games}
...
Each node in Godot has a \"Process Mode\" that defines when it
processes. It can be found and changed under a node\'s
`Node <class_Node>`{.interpreted-text role="ref"} properties in the
inspector.
...
public override void [Ready]{#ready}() { ProcessMode =
...
- **Pausable**: Process the node (and its children in Inherit mode) only
...
By default, all nodes have this property in the \"Inherit\" state. If
the parent is set to \"Inherit\", then the grandparent will be checked
and so on. If a state can\'t be found in any of the grandparents, the
pause state in SceneTree is used. This means that, by default, when the
game is paused every node will be paused. Several things happen when a
node stops processing.
...
connected function to run, even if that function\'s script is attached
to a node that is not currently being processed.
...
func [on_pause_button_pressed]{#on_pause_button_pressed}():

:   get_tree().paused = true show()
...
func [on_close_button_pressed]{#on_close_button_pressed}():
