...
Select a node in the Scene dock then click the add button with the +
symbol.
...
A selected Node from the Scene dock can be added into groups by marking
the checkbox on the left side of the groups in the Groups dock. The node
you had selected when creating a new group will be automatically
checked.
...
func [on_player_spotted]{#on_player_spotted}():
...
public void [OnPlayerDiscovered]{#onplayerdiscovered}() {
