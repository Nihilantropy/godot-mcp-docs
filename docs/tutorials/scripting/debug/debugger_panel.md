# Debugger panel {#doc_debugger_panel}
...
Using the debugger and breakpoints on
`tool scripts <doc_running_code_in_the_editor>`{.interpreted-text
role="ref"} is not currently supported. Breakpoints placed in the script
editor or using the `breakpoint` keyword are ignored. You can use print
statements to display the contents of variables instead.
::::

## Errors

This is where error and warning messages are printed while running the
game.
...
The Visual Profiler is not supported when using the Compatibility
renderer on macOS, due to platform limitations.
::::
...
The monitors are graphs of several aspects of the game while it\'s
running such as FPS, memory usage, how many nodes are in a scene and
more. All monitors keep track of stats automatically, so even if one
monitor isn\'t open while the game is running, you can open it later and
