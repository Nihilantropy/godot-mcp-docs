...
When autoloading a script, a `class_Node`{.interpreted-text role="ref"}
will be created and the script will be attached to it. This node will be
added to the root viewport before any other scenes are loaded.
::::
...
`res://global.gd`. Press **Add** to add it to the autoload list and name
it \"Global\", which is required for scripts to access it by the name
\"Global\":
...
:   var root = get_tree().root \# Using a negative index counts from the
...
> public override void [Ready]{#ready}() { Viewport root =
> GetTree().Root; // Using a negative index counts from the end, so this
> gets the last child node of [root]{.title-ref}. CurrentScene =
> root.GetChild(-1); }
...
:   \# This function will usually be called from a signal callback, \#
    ...
    [deferred_goto_scene.call_deferred]{#deferred_goto_scene.call_deferred}(path)

func [deferred_goto_scene]{#deferred_goto_scene}(path):
...
kind of 2D element just before changing the scene. You can then hide it
just after the scene is changed. This can be used to indicate to the
player that a scene is being loaded.
::::
