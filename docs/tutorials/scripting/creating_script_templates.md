# Creating script templates {#doc_creating_script_templates}
...
configuring the project setting
`Editor > Script > Templates Search Path<class_ProjectSettings_property_editor/script/templates_search_path>`{.interpreted-text
role="ref"}, both via code and the editor.
...
It is possible to customize this behavior by adding meta headers at the
...
file. The directory won\'t be visible throughout the project\'s
filesystem anymore, yet the templates can be modified by an external
text editor anytime.
::::
...
[ready]{#ready}() -\> void: pass \# Replace with function body.

\# Called every frame. \'delta\' is the elapsed time since the previous
frame. func [process]{#process}(delta: float) -\> void: pass
:::
...
using [BINDINGS_NAMESPACE]{#bindings_namespace}\_; using System;

public partial class [CLASS]{#class}\_ : [BASE]{#base}\_ { // Called
when the node enters the scene tree for the first time. public override
void [Ready]{#ready}() { }

> // Called every frame. \'delta\' is the elapsed time since the
> previous frame. public override void [Process]{#process}(double delta)
> { }
...
### Base placeholders
  ...
  `_CLASS_SNAKE_CASE_`     The name of the new class as `snake_case` (used in GDScript
                           only).
  ...
  `_TS_`                   Indentation placeholder. The exact type and number of
                           ...
                           role="ref"} respectively. Can be overridden by the
