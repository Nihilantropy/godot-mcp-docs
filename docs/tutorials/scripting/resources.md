...
`.tscn` or a `.scn` file), an image, a script\... Here are some
...
public override void [Ready]{#ready}() { // Godot loads the Resource
when it executes this line. var texture =
...
private PackedScene [bulletScene]{#bulletscene} =
GD.Load\<PackedScene\>(\"res://Bullet.tscn\");

private void OnShoot() { Node bullet =
[bulletScene.Instantiate]{#bulletscene.instantiate}(); AddChild(bullet);
}
::::
...
1.  Create a new Resource object in the Inspector. This can even be a
...
the Inspector, click the save icon at the top of the Inspector, and
...
Let\'s see some examples. Create a
...
To make the new resource class appear in the Create Resource GUI you
need to provide a class name for GDScript, or use the \[GlobalClass\]
attribute in C#.
::::
...
class_name BotStats extends Resource

\@export var health: int \@export var sub_resource: Resource \@export
...
the inspector. func [init]{#init}(p_health = 0, p_sub_resource = null,
...
> public override void [Ready]{#ready}() { if (Stats is BotStats
> botStats) { GD.Print(botStats.Health); // Prints \'10\'. } } }
...
:   \"GodotBot\": BotStats.new(10), \# Creates instance with 10 health.
    \"DifferentBot\": BotStats.new(20) \# A different one with 20
    health.
...
Godot.Collections.Dictionary\<string, BotStats\> [stats]{#stats} = new
Godot.Collections.Dictionary\<string, BotStats\>();

> public BotStatsTable() { [stats]{#stats}\[\"GodotBot\"\] = new
> BotStats(10); // Creates instance with 10 health.
> [stats]{#stats}\[\"DifferentBot\"\] = new BotStats(20); // A different
> one with 20 health. GD.Print([stats]{#stats}); }
...
script as an extension of their type. This means that trying to assign
an inner class of a script (i.e. using the `class` keyword in GDScript)
won\'t work. Godot will not serialize the custom properties on the
script inner class properly.
...
> public override void [Ready]{#ready}() { var res = new MyResource();
>
> > // This will NOT serialize the \'Value\' property.
