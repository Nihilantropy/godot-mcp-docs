# Using NavigationAgents {#doc_navigation_using_navigationagents}
...
For more advanced uses consider
`doc_navigation_using_navigationpathqueryobjects`{.interpreted-text
role="ref"} over NavigationAgent nodes.
::::
...
- At `path_desired_distance` from the next path position, the agent
  advances its internal path index to the subsequent next path position.
...
- At `path_max_distance` from the ideal path to the next path position,
  the agent requests a new path because it was pushed too far off.
...
`avoidance_enabled` property must be set to `true`.
...
Using the NavigationAgent `avoidance_enabled` property is the preferred
...
override void [Ready]{#ready}() { Rid agent = GetRid(); // Enable
avoidance NavigationServer2D.AgentSetAvoidanceEnabled(agent, true); //
Create avoidance callback
NavigationServer2D.AgentSetAvoidanceCallback(agent,
...
override void [Ready]{#ready}() { Rid agent = GetRid(); // Enable
avoidance NavigationServer3D.AgentSetAvoidanceEnabled(agent, true); //
Create avoidance callback
NavigationServer3D.AgentSetAvoidanceCallback(agent,
...
:   navigation_agent.velocity_computed.connect(Callable([on_velocity_computed]{#on_velocity_computed}))
...
:   \# Do not query when the map has never synchronized and is empty. if
    ...
    [on_velocity_computed]{#on_velocity_computed}(new_velocity)

func [on_velocity_computed]{#on_velocity_computed}(safe_velocity: Vector2) -\> void:
...
func [on_velocity_computed]{#on_velocity_computed}(safe_velocity: Vector2):
...
MovementSpeed { get; set; } = 4.0f; NavigationAgent2D
[navigationAgent]{#navigationagent}; private float
[movementDelta]{#movementdelta};

> public override void [Ready]{#ready}() {
> [navigationAgent]{#navigationagent} =
> GetNode\<NavigationAgent2D\>(\"NavigationAgent2D\");
> [navigationAgent.VelocityComputed]{#navigationagent.velocitycomputed}
> += OnVelocityComputed; }
>
> private void SetMovementTarget(Vector2 movementTarget) {
> [navigationAgent.TargetPosition]{#navigationagent.targetposition} =
> movementTarget; }
>
> public override void [PhysicsProcess]{#physicsprocess}(double delta) {
> // Do not query when the map has never synchronized and is empty. if
> (NavigationServer2D.MapGetIterationId([navigationAgent.GetNavigationMap]{#navigationagent.getnavigationmap}())
> == 0) { return; }
>
> > if
> > ([navigationAgent.IsNavigationFinished]{#navigationagent.isnavigationfinished}())
> > { return; }
> >
> > [movementDelta]{#movementdelta} = MovementSpeed \* (float)delta;
> > Vector2 nextPathPosition =
> > [navigationAgent.GetNextPathPosition]{#navigationagent.getnextpathposition}();
> > Vector2 newVelocity = GlobalPosition.DirectionTo(nextPathPosition)
> > \* [movementDelta]{#movementdelta}; if
> > ([navigationAgent.AvoidanceEnabled]{#navigationagent.avoidanceenabled})
> > { [navigationAgent.Velocity]{#navigationagent.velocity} =
> > newVelocity; } else { OnVelocityComputed(newVelocity); }
>
> }
>
...
\@export var movement_speed: float = 4.0 \@onready var navigation_agent:
NavigationAgent3D = get_node(\"NavigationAgent3D\") var physics_delta:
float
...
:   \# Save the delta for use in
    [on_velocity_computed]{#on_velocity_computed}. physics_delta = delta
    \# Do not query when the map has never synchronized and is empty. if
...
func [on_velocity_computed]{#on_velocity_computed}(safe_velocity: Vector3) -\> void:

:   global_position = global_position.move_toward(global_position +
    safe_velocity, physics_delta \* movement_speed)
:::
...
func [on_velocity_computed]{#on_velocity_computed}(safe_velocity: Vector3):
...
MovementSpeed { get; set; } = 4.0f; NavigationAgent3D
[navigationAgent]{#navigationagent}; private float
[movementDelta]{#movementdelta};
...
> public override void [PhysicsProcess]{#physicsprocess}(double delta) {
> // Do not query when the map has never synchronized and is empty. if
> (NavigationServer3D.MapGetIterationId([navigationAgent.GetNavigationMap]{#navigationagent.getnavigationmap}())
> == 0) { return; }
>
> > if
> > ([navigationAgent.IsNavigationFinished]{#navigationagent.isnavigationfinished}())
