# Using NavigationLayers {#doc_navigation_advanced_using_navigationlayers}
...
:   var region: NavigationRegion2D = get_node(\"NavigationRegion2D\") \#
    ...
    that only considers 2-nd layer regions var path: PackedVector2Array
    = NavigationServer2D.map_get_path( map, start_position,
    target_position, true, path_query_navigation_layers )

static func is_bitmask_inx_enabled([bitmask]{#bitmask}: int, [index]{#index}: int) -\> bool:

:   return [bitmask]{#bitmask} & (1 \<\< [index]{#index}) != 0

static func enable_bitmask_inx([bitmask]{#bitmask}: int, [index]{#index}: int) -\> int:

:   return [bitmask]{#bitmask} \| (1 \<\< [index]{#index})

static func disable_bitmask_inx([bitmask]{#bitmask}: int, [index]{#index}: int) -\> int:
...
public partial class MyNode2D : Node2D { private Rid [map]{#map};
private Vector2 [startPosition]{#startposition}; private Vector2
...
> > NavigationServer2D.MapGetPath( [map]{#map},
> > [startPosition]{#startposition}, [targetPosition]{#targetposition},
> > true, pathQueryNavigationLayers );
>
> }
>
...
:   var region: NavigationRegion3D = get_node(\"NavigationRegion3D\") \#
    ...
    path_query_navigation_layers =
    enable_bitmask_inx(path_query_navigation_layers, 2) \# get a path
    that only considers 2-nd layer regions var path: PackedVector3Array
...
public partial class MyNode3D : Node3D { private Rid [map]{#map};
private Vector3 [startPosition]{#startposition}; private Vector3
[targetPosition]{#targetposition};
...
> > NavigationServer3D.MapGetPath( [map]{#map},
> > [startPosition]{#startposition}, [targetPosition]{#targetposition},
> > true, pathQueryNavigationLayers );
>
> }
