# Using NavigationServer {#doc_navigation_using_navigationservers}
...
functionality for their dimension.
...
The navigation map is still empty or not updated at this point. All
...
:   \# Use call deferred to make sure the entire scene tree nodes are
    setup \# else await on \'physics_frame\' in a [ready]{#ready}()
    might get stuck. custom_setup.call_deferred()
...
public partial class MyNode3D : Node3D { public override void
[Ready]{#ready}() { // Use call deferred to make sure the entire scene
tree nodes are setup // else await on \'physics_frame\' in a
[Ready]{#ready}() might get stuck. CallDeferred(MethodName.CustomSetup);
}
...
> >
> > // Create a procedural navigation mesh for the region. var
> > newNavigationMesh = new NavigationMesh() { Vertices = \[ new
> > Vector3(0.0f, 0.0f, 0.0f), new Vector3(9.0f, 0.0f, 0.0f), new
> > Vector3(0.0f, 0.0f, 9.0f), \], }; int\[\] polygon = \[0, 1, 2\];
