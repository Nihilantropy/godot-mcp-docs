# Connecting navigation meshes {#doc_navigation_connecting_navmesh}
...
[Ready]{#ready}() { // 2D margins are designed to work with 2D \"pixel\"
values. Rid defaultMapRid = GetWorld2D().NavigationMap;
...
[Ready]{#ready}() { // 3D margins are designed to work with 3D world
unit values. Rid defaultMapRid = GetWorld3D().NavigationMap;
