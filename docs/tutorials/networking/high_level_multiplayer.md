...
a `NodePath` in the scene tree, which will override `multiplayer` for
the node at that path and all of its descendants. This allows sibling
nodes to be configured with different peers, which makes it possible to
run a server and a client simultaneously in one instance of Godot.
...
## Remote procedure calls {#doc_high_level_multiplayer_rpcs}
...
public override void [Ready]{#ready}() { if (Multiplayer.IsServer()) {
...
func [on_some_input]{#on_some_input}(): \# Connected to some input.
...
:   multiplayer.peer_connected.connect([on_player_connected]{#on_player_connected})
    multiplayer.peer_disconnected.connect([on_player_disconnected]{#on_player_disconnected})
    multiplayer.connected_to_server.connect([on_connected_ok]{#on_connected_ok})
    multiplayer.connection_failed.connect([on_connected_fail]{#on_connected_fail})
    multiplayer.server_disconnected.connect([on_server_disconnected]{#on_server_disconnected})
...
func [on_player_connected]{#on_player_connected}(id):
[register_player.rpc_id]{#register_player.rpc_id}(id, player_info)

\@rpc(\"any_peer\", \"reliable\") func
[register_player]{#register_player}(new_player_info): var new_player_id
...
func [on_player_disconnected]{#on_player_disconnected}(id):

:   players.erase(id) player_disconnected.emit(id)

func [on_connected_ok]{#on_connected_ok}():

:   var peer_id = multiplayer.get_unique_id() players\[peer_id\] =
    player_info player_connected.emit(peer_id, player_info)

func [on_connected_fail]{#on_connected_fail}():

:   remove_multiplayer_peer()

func [on_server_disconnected]{#on_server_disconnected}():

:   remove_multiplayer_peer() players.clear() server_disconnected.emit()
...
> Godot.Collections.Dictionary\<string, string\>\> [players]{#players} =
> new Godot.Collections.Dictionary\<long,
...
> Godot.Collections.Dictionary\<string, string\>
> [playerInfo]{#playerinfo} = new Godot.Collections.Dictionary\<string,
> string\>() { { \"Name\", \"PlayerName\" }, };
>
> private int [playersLoaded]{#playersloaded} = 0;
>
> public override void [Ready]{#ready}() { Instance = this;
...
> > if (error != Error.Ok) { return error; }
> >
> > Multiplayer.MultiplayerPeer = peer; [players]{#players}\[1\] =
> > [playerInfo]{#playerinfo}; EmitSignal(SignalName.PlayerConnected, 1,
> > [playerInfo]{#playerinfo}); return Error.Ok;
>
> }
>
> private void RemoveMultiplayerPeer() { Multiplayer.MultiplayerPeer =
> null; [players.Clear]{#players.clear}(); }
>
> // When the server decides to start the game from a UI scene, // do
...
> \[Rpc(MultiplayerApi.RpcMode.AnyPeer,CallLocal = true,TransferMode =
> MultiplayerPeer.TransferModeEnum.Reliable)\] private void
> PlayerLoaded() { if (Multiplayer.IsServer()) {
> [playersLoaded]{#playersloaded} += 1; if
> ([playersLoaded]{#playersloaded} == [players.Count]{#players.count}) {
> GetNode\<Game\>(\"/root/Game\").StartGame();
> [playersLoaded]{#playersloaded} = 0; } } }
>
> // When a peer connects, send them my player info. // This allows
> transfer of all desired data for each player, not only the unique ID.
> private void OnPlayerConnected(long id) { RpcId(id,
> MethodName.RegisterPlayer, [playerInfo]{#playerinfo}); }
>
...
> [players]{#players}\[newPlayerId\] = newPlayerInfo;
> EmitSignal(SignalName.PlayerConnected, newPlayerId, newPlayerInfo); }
>
> private void OnPlayerDisconnected(long id) {
> [players.Remove]{#players.remove}(id);
> EmitSignal(SignalName.PlayerDisconnected, id); }
>
> private void OnConnectOk() { int peerId = Multiplayer.GetUniqueId();
> [players]{#players}\[peerId\] = [playerInfo]{#playerinfo};
...
be a player. You\'ll also have to modify the game starting mechanism so
that the first player who joins can start the game.
::::
