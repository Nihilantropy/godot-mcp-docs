# Using WebSockets {#doc_websocket}
...
a server. Before that, browsers used to only support HTTP requests,
which aren\'t well-suited for bidirectional communication.

The protocol is message-based and a very powerful tool to send push
notifications to browsers. It has been used to implement chats,
turn-based games, and more. It still uses a TCP connection, which is
good for reliability but not for latency, so it\'s not good for
real-time applications like VoIP and fast-paced games (see
...
than a raw TCP connection, WebSocket started to spread outside the
