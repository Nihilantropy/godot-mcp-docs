# Using Fonts {#doc_gui_using_fonts}
...
Alternatively, you can import any image to be used as a bitmap font. To
do so, select the image in the FileSystem dock, go to the Import dock,
change its import type to **Font Data (Image Font)** then click
**Reimport**:
...
<figcaption>Changing import type to <strong>Font Data (Image
...
If you need finer control over character spacing than what the
**Character Margin** options provide, you have more options.

For one, **Character Ranges** supports 3 additional arguments after the
specified range of characters. These additional arguments control their
positioning and spacing. They represent space advance, X axis offset,
and Y axis offset in that order. They will change the space advance and
offset of each character by the amount of pixels written. Space advance
is most useful if, for example, your lowercase letters are thinner than
your uppercase letters.

<figure class="align-center">
<img src="img/using_fonts_bitmap_font_advance_offsets_diagram.webp"
alt="img/using_fonts_bitmap_font_advance_offsets_diagram.webp" />
<figcaption>Do note that the offsets can cause your text to be cropped
off the edge of your label boundaries.</figcaption>
</figure>

Secondly, you can also set up **Kerning Pairs** for individual
characters. Specify your kerning pair by typing two sets of characters
separated by a space, then followed by another space, a number to
specify how many extra/less pixels to space those two sets of characters
when placed next to each other.

![](img/using_fonts_bitmap_kerning_pairs_example.webp)

If needed, your kerning pair characters can be specified by Unicode
character code by entering `\uXXXX` where XXXX is the hexadecimal value
...
creating a `class_LabelSettings`{.interpreted-text role="ref"} resource
that you reuse across Label nodes. This resource takes priority over
`theme properties <doc_gui_skinning>`{.interpreted-text role="ref"}.
::::

## Advanced font features

### Antialiasing {#doc_using_fonts_antialiasing}
...
`doc_using_fonts_opentype_font_features`{.interpreted-text role="ref"},
but this is not covered here for brevity reasons.
::::
...
- **Bottom:** Additional spacing below glyphs. This is used for
  multiline text, but also to calculate the minimum size of controls
  such as `class_Label`{.interpreted-text role="ref"} and
  `class_Button`{.interpreted-text role="ref"}.
...
at runtime. This has 2 benefits:
...
Create a `class_SystemFont`{.interpreted-text role="ref"} resource in
...
\"standard\" default font for the system:
  ...
  -------------- ----------- ----------- ----------------- -----------------
  `sans-serif`   Arial       Helvetica   *Handled by       Roboto / Noto
                                         fontconfig*       Sans

  `serif`        Times New   Times       *Handled by       Noto Serif
                 Roman                   fontconfig*       

  `monospace`    Courier New Courier     *Handled by       Droid Sans Mono
                                         fontconfig*       

  `cursive`      Comic Sans  Apple       *Handled by       Dancing Script
                 MS          Chancery    fontconfig*       

  `fantasy`      Gabriola    Papyrus     *Handled by       Droid Sans Mono
                                         fontconfig*       
  --------------------------------------------------------------------------
...
### Font prerendering {#doc_using_fonts_font_prerendering}
