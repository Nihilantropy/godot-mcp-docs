# The JavaScriptBridge singleton {#doc_web_javascript_bridge}
...
As you can see, by wrapping JavaScript objects into `JavaScriptObject`
...
Callback methods created via
`JavaScriptBridge.get_interface() <class_JavaScriptBridge_method_get_interface>`{.interpreted-text
role="ref"} (`_my_callback` in the above example) **must** take exactly
one `Array<class_Array>`{.interpreted-text role="ref"} argument, which
is going to be the JavaScript [arguments
object](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/arguments)
converted to an array. Otherwise, the callback method will not be
called.
::::
