...
- By declaring that the Activity will handle these configuration events
...
- If using `gradle`, include the following `aaptOptions` configuration
  ...
  - If your build system does not support including hidden directories,
    you can configure the Godot project to not use hidden directories by
    deselecting
    `Application > Config > Use Hidden Project Data Directory<class_ProjectSettings_property_application/config/use_hidden_project_data_directory>`{.interpreted-text
