...
private double [timeBegin]{#timebegin}; private double
[timeDelay]{#timedelay};

public override void [Ready]{#ready}() { [timeBegin]{#timebegin} =
Time.GetTicksUsec(); [timeDelay]{#timedelay} =
AudioServer.GetTimeToNextMix() + AudioServer.GetOutputLatency();
GetNode\<AudioStreamPlayer\>(\"Player\").Play(); }

public override void [Process]{#process}(double delta) { double time =
(Time.GetTicksUsec() - [timeBegin]{#timebegin}) / 1000000.0d; time =
Math.Max(0.0d, time - [timeDelay]{#timedelay});
