# Text to speech {#doc_text_to_speech}
...
These libraries are installed by default on Windows, macOS, Web, Android
and iOS, but not on all Linux distributions. If they are not present,
text-to-speech functionality will not work. Specifically, the
`tts_get_voices()` method will return an empty list, indicating that
there are no usable voices.
...
+---------+------------------------------------------------------------+
| **Arch  |     pacman -S speech-dispatcher festival espeakup          |
| Linux** |                                                            |
+---------+------------------------------------------------------------+
