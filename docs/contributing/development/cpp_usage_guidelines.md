# C++ usage guidelines {#doc_cpp_usage_guidelines}
...
[master]{.title-ref}, your code can\'t use C++17 features. Instead, your
code must be able to be built with a C++14 compiler.
...
### Standard Template Library {#doc_cpp_godot_types}
...
in the `doc_core_types`{.interpreted-text role="ref"} documentation.
...
The `auto` keyword can be used in some special cases, like C++ lambda or
Objective-C block definitions and C++ templates. Please ask before using
templates with `auto` in a pull request.
...
We chose to forbid `auto` in all other cases. Thank you for your
understanding.
...
Starting with 4.5, all files now use the `#pragma once` directive, as
they improve readability and declutter macros. Use of `#ifdef`-based
include guards are now actively discouraged.

### `try`-`catch` blocks

C++ style exception handling using `try` and `catch` blocks is
forbidden. This restriction is in place for several reasons, including
performance, binary size and code complexity. Use
`doc_common_engine_methods_and_macros_error_macros`{.interpreted-text
role="ref"} instead.
