# Compiling for Windows {#doc_compiling_for_windows}
...
- [Python 3.8+](https://www.python.org/downloads/windows/). **Make sure
...
with MinGW compared to MSVC. Therefore, it\'s recommended to use MinGW
to produce builds that you distribute to players.
...
`python misc/scripts/install_d3d12_sdk_windows.py` in the Godot source
repository. After running this script, add the `d3d12=yes` SCons option
to enable Direct3D 12 support. This will use the default paths for the
various dependencies, which match the ones used in the script.
...
| **Fedora**   | ``` shell                                             |
|              | dnf                                                   |
|              |  install mingw64-gcc-c++ mingw64-winpthreads-static \ |
|              |                                                       |
...
instead want to configure your binaries as custom export templates in
the project export menu. You must have **Advanced Options** enabled to
set this.
