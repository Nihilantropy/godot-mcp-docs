# Writing guidelines {#doc_docs_writing_guidelines}
...
## 7 rules for clear English {#doc_docs_writing_guidelines_clear_english_rules}
...
### Dynamic vs static typing {#doc_docs_writing_guidelines_dynamic_typing}
...
### Use real-world code examples where appropriate {#doc_docs_writing_guidelines_real_world_code_example}
...
## Common vocabulary to use in Godot\'s documentation {#doc_docs_writing_guidelines_common_vocabulary}
...
## Keyboard shortcut guidelines {#doc_docs_writing_guidelines_keyboard_shortcuts}
...
## Manual style guidelines {#doc_docs_writing_guidelines_manual_style}

Follow these formatting and style guidelines when writing the manual.

Use your best judgement. If you can write more clearly by breaking one
of these guidelines, please do! But remember that the guidelines exist
for a reason.
...
already making changes to a paragraph or section of the docs, update it
to follow these standards. Avoid making unrelated changes that *only*
update style, since every change will require the paragraph to be
re-translated.
::::
...
There are a few styles that the manual uses.
  ...
  *Italics*    `*text*`        Used for emphasis. Used for introducing
                               new terms.

  **Bold**     `**text**`      Used for emphasis, and for editor UI like
                               menus and windows.

  `Code`       `text`          Used for variable names, literal values,
                               and code snippets. `code` is used in many
                               cases where you would use \"quoted
                               plaintext\" in typical English.

  \"Quotes\"   `"text"`        Used for some literal or quoted values. In
                               many cases, another style is preferred.
  -----------------------------------------------------------------------

### Emphasis

Use either **bold style** or *italic style* to emphasize words or
sentences. In most cases, either **bold** or *italics* is fine. Use
whichever seems best, or whatever the page already uses.

Prefer using **bold style** for simple emphasis.

> - Do **not** close the window without saving first.

Use *italic style* or to emphasize one word in the context of a
sentence.

> - You can *add* a node to the scene (but you can\'t connect one).
> - You can add a *node* to the scene (but you can\'t add a resource).
> - You can add a node to the *scene* (but you can\'t add one to a
>   resource).
...
### Literals {#doc_docs_writing_guidelines_literals}
...
> - Vector literals like `(0.0, 0.0)`, `(0.5, -0.5, 0.5)`, or
>   `(1.0, 2.0, 3.0, 4.0)`.

### Classes, properties, and methods {#doc_docs_writing_guidelines_class_properties_methods}

Link to classes the first time that you mention them in a page. After
the first mention, use `code style`. For common classes, like `Node`,
`Control`, or `Viewport`, you can also use plaintext.

Link to class members (properties, methods, enums, and constants) the
first time that you mention them in a page. After the first mention, use
`code style`. If the class member is very common, like a Node2D\'s
`position`, you don\'t have to link.

When discussing properties in the context of the inspector, use **bold
style** instead.

### Editor UI {#doc_docs_writing_guidelines_editor_ui}

Use **bold style** for editor UI, including window titles, menus,
buttons, input fields, inspector properties, and inspector sections. Use
the exact capitalization that the editor uses.

> - Open the **Editor Settings** window.
> - Press the **Confirm** button.
> - Change the node\'s **Transform \> Position** property to `(0, 0)`.
> - In the **Project Settings** window, enable the **Advanced Settings**
>   toggle.

Use **Bold \> With \> Separators** when describing sequence of menus
that the reader must navigate. Use `>` as a separator. You can omit
ellipses in menu names.

> - In **Project \> Project Settings \> Input Map**, add a new input
>   action.
...
### Project settings {#doc_docs_writing_guidelines_project_settings}

Link to individual project settings. Either include the section and
subsection in the link itself, or include the section and subsection
separately from the link. Since long links are not split into multiple
lines when the page is rendered, prefer splitting the setting name and
the section when the link is long.

> - Set the
>   `Application > Run > Max FPS<class_ProjectSettings_property_application/run/max_fps>`{.interpreted-text
>   role="ref"} setting to `60`.
> - In the project settings under **Application \> Run**, set
>   `Max FPS<class_ProjectSettings_property_application/run/max_fps>`{.interpreted-text
>   role="ref"} to `60`.
> - In **Project Settings \> Application \> Run**, set
>   `Max FPS<class_ProjectSettings_property_application/run/max_fps>`{.interpreted-text
>   role="ref"} to `60`.

### Code blocks and adominitions

Code blocks should use the `::` syntax on its own line. Do not use the
short form where it\'s at the end of a paragraph, as the trailing `::`
would be included in the localizable string otherwise. Weblate would
show a warning due to the trailing `::`, and if the localized string
does not end with `::`, it would break the code block entirely in the
translated manual.
...
As for admonitions (note, warning, etc. blocks), you should write the
admonition tag on its own line, then the admonition\'s contents indented
by 4 spaces. Do not write the first line of the contents on the same
line as the admonition tag.
...
### Manually wrapping lines {#doc_docs_writing_guidelines_manually_wrapping_lines}

In the manual, lines must be manually wrapped to no more than 80-100
characters per line. However, links must not be split into multiple
lines, and can exceed 100 characters. Tables can also exceed 100
characters.

When making small changes, you don\'t need to manually re-wrap the whole
paragraph, as long as the lines don\'t exceed 100 characters.
...
characters. For example, in Visual Studio Code, you can add the
following to your `settings.json` to add rulers at 80 and 100
characters:
...
Note that headers have no inherent meaning. In reStructuredText, headers
are parsed based on the order that they initially appear within a page.
Make sure that if you use an `h3` section header (`~~~`), you include an
`h2` sub-section header (`---`) first.

See the [Sphinx
documentation](https://www.sphinx-doc.org/en/master/usage/restructuredtext/basics.html#sections)
and the [reStructuredText
documentation](https://docutils.sourceforge.io/docs/ref/rst/restructuredtext.html#sections)
for more information.

## When to refer to a specific Godot version {#doc_docs_writing_guidelines_specific_version}
...
Much of the manual involves describing a sequence of UI actions in the
editor, like clicking a button, opening a menu, or setting a property in
the inspector. To keep formatting standardized, we use custom Sphinx
roles for UI elements.

The following roles are defined:

- `:button:` A button, toggle, or other clickable UI element. If the
  reader is meant to click on it, and it\'s not a menu, use this.
  Renders as `bold, with a background`{.interpreted-text role="button"}.
- `:menu:` A series of menus to click through. When listing a series of
  menus, separate them with `>`. Renders as
  `bold, with a background`{.interpreted-text role="menu"}.
- `:inspector:` A property *in the inspector*. When describing a
  property in *code*, instead either use `code style` or link to the
  property, as described earlier. Renders as `bold`{.interpreted-text
  role="inspector"}.
- `:ui:` A role for any other editor UI elements. Use this if you would
  have otherwise just used **bold style**. Use this for input fields,
  docks, tabs, windows, bottom panels, etc. Also used for nested project
  settings or inspector sections. Renders as `bold`{.interpreted-text
  role="ui"}.

The first two roles, `:button:` and `:menu:` are used for editor UI that
the reader is meant to click on, and they use an attention-grabbing
visual style. The other roles, `:inspector:` and `:ui:`, are used for
other UI and show up often in text, so they just use bold text to be
less distracting.

Our custom roles are inspired by the Sphinx
...
roles. However, we use our own implementation to better match the
specific needs of Godot\'s documentation, using [custom RST
roles](https://docutils.sourceforge.io/docs/ref/rst/directives.html#custom-interpreted-text-roles)
and some custom CSS.

### Examples

These are some example sections that use the roles, in context. Check
the source of this page to see which roles are used.

#### Adding a sprite and setting some properties

In the `Scene`{.interpreted-text role="ui"} dock, click
`2D Scene`{.interpreted-text role="button"} to create a new scene.

Add a new `Sprite2D <class_Sprite2D>`{.interpreted-text role="ref"} to
the scene by right-clicking on the root node and choosing
`Add Child Node...`{.interpreted-text role="button"}. In the
`Create New Node`{.interpreted-text role="ui"} window, search for
\"Sprite2D\", select it, and then click `Create`{.interpreted-text
role="button"}.

On the sprite, under `Offset`{.interpreted-text role="ui"}, set
`Offset`{.interpreted-text role="inspector"} to `(16, 32)` and enable
`Flip H`{.interpreted-text role="inspector"}. Set
`Animation > HFrames`{.interpreted-text role="inspector"} to `10`. In
`CanvasItem > Visibility`{.interpreted-text role="ui"}, set the
`Modulate`{.interpreted-text role="inspector"} color to `ff0000`.
...
Don\'t forget to save your scene in
`Scene > Save Scene...`{.interpreted-text role="menu"}. When the
`Save Scene As...`{.interpreted-text role="ui"} window pops up, enter
\"my_scene.tscn\" in the `File`{.interpreted-text role="ui"} field, then
click `Save`{.interpreted-text role="button"}.
::::

#### Setting project settings

Go to `Project > Project Settings`{.interpreted-text role="menu"}, then
select the
`Max FPS <class_ProjectSettings_property_application/run/max_fps>`{.interpreted-text
role="ref"} setting under `Application > Run`{.interpreted-text
role="ui"}. Don\'t forget to click the
`Advanced Settings`{.interpreted-text role="button"} toggle. Then, in
`Filter Settings`{.interpreted-text role="ui"}, search for \"physics\".
Under `Physics > 3D > Solver`{.interpreted-text role="ui"}, set
`Solver Iterations`{.interpreted-text role="inspector"} to `16`.

#### All styles in context

Use this section to see how the custom roles look, particularly within
admonitions.

Built-in styles: `code`, **bold**, and *italics*. Built-in roles:
`kbd`{.interpreted-text role="kbd"},
`ref <doc_about_intro>`{.interpreted-text role="ref"},
`ref <class_node>`{.interpreted-text role="ref"}. Custom roles:
`button`{.interpreted-text role="button"},
`menu > submenu`{.interpreted-text role="menu"},
`inspector`{.interpreted-text role="inspector"}, `ui`{.interpreted-text
role="ui"}.
