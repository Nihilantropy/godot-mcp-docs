# Creating documentation images and videos {#docs_image_guidelines}
...
on KDE Plasma the program Spectacle is used for taking screenshots. If
your distribution doesn\'t come with one by default try searching its
package repository, or Flathub if that\'s supported.
...
### Format conversion {#doc_docs_image_guidelines_format_conversion}
...
GIFs should be avoided. Their compression is inefficient and they only
support a 256-color palette with 1-bit transparency.
::::

### Cropping {#doc_docs_image_guidelines_cropping}
...
### Scaling down an image {#doc_docs_image_guidelines_scaling_down}
...
To save an image as webp if it isn\'t already one, Go to **File \> Save
...
### Outlines, arrows and text {#doc_docs_image_guidelines_outlines}
...
## Videos {#doc_docs_image_guidelines_videos}
...
specify *any* of these flags. The `:align: default` flag should always
be specified.
