...
not a bad way to learn, as long as you\'re curious and don\'t hesitate
to question your search engine when lost, so we will give you the basic
...
That\'s a lot to memorize! Don\'t worry, just check this cheat sheet
...
frequently, your local `upstream/master` branch may become outdated, so
you can update it with `git fetch upstream master`. Contrarily to
